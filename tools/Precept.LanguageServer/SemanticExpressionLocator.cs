...
internal static class SemanticExpressionLocator
{
    internal static bool TryFindExpressionAt(SemanticIndex index, Position position, out TypedExpression expression)
    {
        var candidate = EnumerateTypedExpressions(index)
            .Where(candidate => Contains(candidate.Span, position))
            .OrderBy(candidate => GetSpanWidth(candidate.Span))
            .ThenByDescending(candidate => candidate.Span.StartLine)
            .ThenByDescending(candidate => candidate.Span.StartColumn)
            .FirstOrDefault();
    ...
    internal static bool TryFindFunctionAt(Compilation compilation, Position position, out IReadOnlyList<FunctionMeta> overloads)
    {
        overloads = [];
        ...
        var nextIndex = FindNextSignificantToken(tokens, tokenIndex + 1);
        if (nextIndex < 0 || tokens[nextIndex].Kind != TokenKind.LeftParen)
        {
            return false;
        }
        ...
        if (!TryFindExpressionAt(compilation.Semantics, position, out var expression)
            || expression is not TypedFunctionCall functionCall)
        {
            return false;
        }

        var matches = Functions.FindByName(token.Text).ToArray();
        if (matches.Length == 0)
        {
            return false;
        }

        var resolved = Functions.GetMeta(functionCall.ResolvedFunction);
        if (!string.Equals(resolved.Name, token.Text, System.StringComparison.Ordinal))
        {
            return false;
        }
    ...
    internal static bool TryFindAccessorAt(Compilation compilation, Position position, out TypeMeta ownerType, out TypeAccessor accessor)
    {
        ownerType = null!;
        accessor = null!;
        ...
        if (!TryFindExpressionAt(compilation.Semantics, position, out var expression)
            || expression is not TypedMemberAccess memberAccess)
        {
            return false;
        }

        if (!string.Equals(memberAccess.ResolvedAccessor.Name, tokens[tokenIndex].Text, System.StringComparison.Ordinal))
        {
            return false;
        }

        ownerType = Types.GetMeta(memberAccess.Object.ResultType);
        accessor = memberAccess.ResolvedAccessor;
        return true;
    }
    ...
    private static bool IsStructural(TokenKind kind) => kind is TokenKind.NewLine or TokenKind.EndOfSource;
