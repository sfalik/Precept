...
4. **Invariants** — identify rules that must hold in every state (e.g., `amount >= 0`).
5. **State constraints** — identify assertions that must be true when entering or remaining in a specific state (e.g., `in Approved assert DocumentsVerified`).
6. **Event assertions** — identify validation rules on event arguments (e.g., `on Submit assert Amount > 0`).
...
8. **Edit declarations** — identify which fields are directly editable in which states (`in <State> edit Field1, Field2`).
...
### Catch-all events
Use `from any on <Event>` for events that apply regardless of state (e.g., incoming data updates):
```
