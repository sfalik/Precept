...
- **`noHaiku: true` is a hard ban.** If `.squad/config.json` sets `noHaiku` to `true`, the coordinator MUST NEVER select any Haiku model, MUST NEVER use Haiku in fallback chains, and MUST replace fast-tier defaults with the newest allowed non-Haiku model.
...
**Layer 3 — Task-Aware Auto-Selection:** Use the governing principle: **latest appropriate model first.** Prefer the newest suitable model version unless the user explicitly asked to optimize for cost. Match the agent's task to determine output type, then select accordingly:

| Task Output | Model | Tier | Rule |
|-------------|-------|------|------|
| Writing code (implementation, refactoring, test code, bug fixes) | `claude-sonnet-4.6` | Standard | Prefer the newest high-quality coding model by default. |
| Writing prompts or agent designs (structured text that functions like code) | `claude-sonnet-4.6` | Standard | Prompts are executable — treat like code and use the newest standard model. |
| NOT writing code (docs, planning, triage, logs, changelogs, mechanical ops) | `gpt-5.4` | Fast | Prefer the newest general-purpose model instead of older budget defaults. |
| Visual/design work requiring image analysis | `claude-opus-4.6` | Premium | Vision capability required; use the newest premium vision-capable model. |

**Role-to-model mapping** (applying latest-first principle):

| Role | Default Model | Why | Override When |
|------|--------------|-----|---------------|
| Core Dev / Backend / Frontend | `claude-sonnet-4.6` | Writes code — quality first | Heavy code gen → `gpt-5.2-codex` |
| Tester / QA | `claude-sonnet-4.6` | Writes test code — quality first | Simple test scaffolding → `gpt-5.4` |
| Lead / Architect | auto (per-task) | Mixed: review/planning can still use the newest models | Architecture proposals → premium; triage/planning → `gpt-5.4` |
| Prompt Engineer | auto (per-task) | Mixed: prompt design is like code, research is not | Prompt architecture → sonnet; research/analysis → `gpt-5.4` |
| Copilot SDK Expert | `claude-sonnet-4.6` | Technical analysis that often touches code | Pure research → `gpt-5.4` |
| Designer / Visual | `claude-opus-4.6` | Vision-capable model required | — (never downgrade — vision is non-negotiable) |
| DevRel / Writer | `gpt-5.4` | Docs and writing — use the newest general model | — |
| Scribe / Logger | `gpt-5.4` | Mechanical file ops should still respect the latest-model preference | — |
| Git / Release | `gpt-5.4` | Mechanical ops should still respect the latest-model preference | — |
...
Standard: claude-sonnet-4.6 → gpt-5.4 → gpt-5.2-codex → claude-sonnet-4.5 → (omit model param)
Fast:     gpt-5.4 → gpt-5-mini → gpt-5.1-codex-mini → (omit model param)
```
