...
- **"Does the parser hardcode a set of token kinds that the catalog already knows?"** Before writing any `FrozenSet<TokenKind>`, lookahead check (`Peek(n).Kind == X`), or disambiguation condition in the parser, check whether a catalog encodes this distinction. `Constructs.ByLeadingToken` knows which tokens lead each construct. `DisambiguationEntry.DisambiguationTokens` (via `ConstructMeta.Entries`) knows which tokens distinguish constructs with the same leader. `Modifiers`, `Actions`, `Types`, and `Operators` catalogs cover their respective token sets. Derive from the catalog — never maintain a parallel set in the parser.
- **"Does any pipeline stage switch on a `*Kind` enum value to apply per-member behavior?"** The smell is `kind switch { FooKind.Bar => ..., FooKind.Baz => ... }` or a hand-maintained array of specific members where the *reason* each branch exists is "the language says so." That reason belongs in catalog metadata. Switching on a DU **subtype** is correct — the subtype is the metadata shape, not a classification axis. Switching on a **catalog member's enum identity** to dispatch per-member behavior is the smell; the behavior belongs in the catalog entry.
