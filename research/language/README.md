...
| [expressiveness/business-string-ordering-use-cases.md](./expressiveness/business-string-ordering-use-cases.md) | CI string ordering survey — 7-category cross-platform analysis. Verdict: no business need for CI ordering (`~<`/`~>`). |
| [expressiveness/string-ordering-vs-ordered-choice.md](./expressiveness/string-ordering-vs-ordered-choice.md) | Non-CI string ordering vs ordered choice — scenario-by-scenario comparison. Verdict: ordered choice covers discrete ranks; does not engage with standardized code systems. |
| [expressiveness/string-ordering-architectural-analysis.md](./expressiveness/string-ordering-architectural-analysis.md) | Architectural analysis of string ordering; corpus survey shows zero current usage. Verdict: remove doc claim (correct for arbitrary strings). |
| [expressiveness/string-ordering-gap-analysis.md](./expressiveness/string-ordering-gap-analysis.md) | Gap analysis: fixed-format standardized codes (ICD-10, ZIP, NAICS, CPT) where ordered choice is infeasible and ordinal is correct. Initial verdict (since revised by the external + broad surveys): do not implement; the theoretical gap exists but has no observed demand. Final synthesis: `primitive-types.md` § String Ordering — Out of Scope. |
| [expressiveness/string-ordering-external-survey.md](./expressiveness/string-ordering-external-survey.md) | External survey: do real practitioners use string `<`/`>` on code fields in Excel, Airtable, Drools, FEEL/DMN, CQL, healthcare payer systems? Verdict: demand signal is absent; real-world standard is code set membership, not string range comparison. Revises gap analysis recommendation. |
| [expressiveness/string-ordering-broad-use-cases.md](./expressiveness/string-ordering-broad-use-cases.md) | Broad cross-domain survey (12 domains: CRM, manufacturing, finance, logistics, etc.) — do domain experts use string `<`/`>` as a rule predicate anywhere? Verdict: no domain provides a demand signal; serial/lot and postal-code ranges are real but handled via from/to data models, not ordering operators. Confirms external survey conclusion. |
| [rational-vs-decimal-exact-numeric-representation.md](./rational-vs-decimal-exact-numeric-representation.md) | Neutral survey of native exact non-integer representation (p/q rational vs arbitrary-precision decimal) across 11 languages, grounding a possible collapse of the three numeric lanes toward a single exact type. Key tensions: transcendental gap blocks "drop float"; rationals help the prover (closed under ÷, SMT-Real precedent) but finance/regulation standardized on decimal; only Raku makes rational the default literal. Four tradeoff clusters, no decision. Feeds a forthcoming numeric-lane `/design`. |
| [business-fraction-vs-decimal-notation.md](./business-fraction-vs-decimal-notation.md) | Evidence survey on how exact non-integer quantities are **notated** (as distinct from stored) in business, legal, financial and regulatory practice, and what domain-expert authoring tools accept and display. Grounds the claim the rational-vs-decimal survey asserted without citation. Verdict: neither "fractions are programmer culture" nor its inverse holds — the split is by **kind of quantity** (fractions for definitionally-exact shares of a whole; decimals for measured/priced/computed magnitudes). Notable legs: OMG DMN/FEEL has no fraction literal (decimal128 backing); SAP and Microsoft Dynamics independently store UOM conversion as a genuine exact integer ratio pair, but posted financial amounts never are; SEC decimalization covers equities/options only while Treasuries still quote in 32nds; NIST HB44 explains the trades fraction/decimal split by measuring instrument, not audience. Comprehension literature does not transfer. No decision proposed. |
