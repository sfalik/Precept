...
- **Principle #2 (English-ish but not English):** The DSL names — `date`, `time`, `instant`, `duration`, `period`, `timezone` — communicate exactly what the data is. The bare name is the default (no timezone); `zoneddatetime` is the marked form. `field FiledAt as instant` needs no comment. Postfix units extend this: `DueDate + 30 days` reads as domain prose.
...
**After** — with temporal types (v4: postfix units + single-quoted literals + bare English naming):
...
**After** — complete rule in the contract (v4: dot-accessor mediation, `date + period → date`):
...
| `PT0.5S` | Not directly expressible — temporal postfix units require integer expressions (Decision #28). Use millisecond-precision via `500 milliseconds` if sub-second support is added, or model as a `duration` field with a computed value. |
...
**Integer requirement:** The expression inside a paren postfix (and the literal in a bare postfix) must evaluate to an integer. `(0.5) days` is a compile error: *"Unit values must be whole numbers. Use smaller units for fractions: `12 hours` for half a day."* NodaTime's `Period.FromDays()`, `Period.FromMonths()`, etc. all take `int`. This restriction applies to temporal unit keywords; future non-temporal postfix quantities may support non-integer expressions. See Decision #28.
...
| `GraceDays days` | Wrap field references in parentheses when used with units. Use `(GraceDays) days`. |
...
- **Default-position IntelliSense:** Normalize completions across all temporal field types. After `default `, offer the canonical starter for the declared field type: `date/time/instant/datetime/timezone/zoneddatetime` get single-quoted typed constants, `duration/period` get postfix-unit quantity snippets.
...
### 28. Temporal postfix units require integer expressions (locked v4)

- **Why:** NodaTime's `Period.FromDays(int)`, `Period.FromMonths(int)`, `Period.FromYears(int)`, `Period.FromWeeks(int)`, `Period.FromHours(long)`, `Period.FromMinutes(long)`, `Period.FromSeconds(long)`, and `Duration.FromHours(double)`, `Duration.FromDays(double)` etc. have mixed signatures — some take `int`, some `long`, some `double`. Rather than exposing this inconsistency, Precept standardizes: all temporal postfix unit expressions require an integer. `(0.5) days` is a compile error. `(GraceDays) months` requires `GraceDays` to be typed `integer`. This is consistent with how temporal quantities work in practice — "2.5 months" is ambiguous (is the .5 half a 28-day month or a 31-day month?), while "2 months + 15 days" is precise.
- **Scope:** This restriction applies to **temporal** unit keywords (`days`, `months`, `years`, `weeks`, `hours`, `minutes`, `seconds`). The postfix grammar mechanism is a general language expansion joint (noted in the proposal's "Expansion Joint" section). Future non-temporal postfix quantities (e.g., currency, measurement units) may support non-integer expressions if their backing types accept them.
...
- **Tradeoff:** `PT0.5S` (half a second) is not directly expressible via postfix. The author would need `500 milliseconds` if sub-second support is added, or model as a computed `duration` field.
...
- `datetime` arithmetic via `LocalDateTime.Plus(Period)`. Time-unit postfixes resolve to `period` in datetime context (Decision #9, #27) — no duration bridging needed.
...
- [ ] `field X as date nullable` parses and type-checks
- [ ] `field X as date default '2026-06-01'` parses and type-checks
- [ ] `field X as date default '2026-06-01' nullable` parses and type-checks
- [ ] Numeric constraints on `date` (`nonnegative`, `min`, `max`, `maxplaces`, `minlength`, `maxlength`) are compile errors

#### Typed constants

- [ ] `'2026-03-15'` validates at compile time as a `date` (content shape `YYYY-MM-DD`)
- [ ] `'2026-02-30'` is a compile error: "Invalid date: February 30 does not exist."
- [ ] `'2026-02-29'` in a non-leap-year is a compile error
- [ ] `'2024-02-29'` validates successfully (2024 is a leap year)
- [ ] `'2026-13-01'` is a compile error (invalid month)
- [ ] `'2026-00-15'` is a compile error (invalid month)
- [ ] `'01/15/2026'` is a compile error: "Dates must be written as YYYY-MM-DD. Use `'2026-01-15'`."
- [ ] `field X as date default "2026-01-01"` is a compile error: "Date values use single quotes, not double quotes. Use `default '2026-01-01'`."
...
- [ ] `date == date → boolean` (NodaTime default behavior, thin wrapper)
- [ ] `date != date → boolean` (NodaTime default behavior, thin wrapper)
- [ ] `date < date → boolean` (ISO calendar ordering, NodaTime default behavior)
- [ ] `date > date → boolean` (ISO calendar ordering, NodaTime default behavior)
- [ ] `date <= date → boolean` (NodaTime default behavior, thin wrapper)
- [ ] `date >= date → boolean` (NodaTime default behavior, thin wrapper)
- [ ] Cross-type comparison (`date == time`, `date < instant`, etc.) is a type error
...
- [ ] `date.month → integer` (1–12)
- [ ] `date.day → integer` (1–31)
- [ ] `date.dayOfWeek → integer` (ISO: Monday=1, Sunday=7)

#### Constraints

- [ ] `nullable` supported
- [ ] `default '...'` supported (single-quoted date)
- [ ] `nonnegative` on `date` is a compile error
- [ ] `min` / `max` on `date` is a compile error
- [ ] `minlength` / `maxlength` on `date` is a compile error

#### Serialization

- [ ] JSON round-trip as ISO 8601 string: `"2026-03-15"`
...
- [ ] `field X as time nullable` parses and type-checks
- [ ] `field X as time default '09:00:00'` parses and type-checks
- [ ] `field X as time default '14:30'` parses (seconds optional, implies `:00`)

#### Typed constants

- [ ] `'14:30:00'` validates at compile time as a `time` (content shape `HH:mm:ss`)
- [ ] `'14:30'` validates as `time` (seconds optional)
- [ ] `'25:00:00'` is a compile error: "Invalid time: hours must be 0–23."
- [ ] `'14:60:00'` is a compile error (invalid minute)
- [ ] `'14:30:60'` is a compile error (invalid second)
...
- [ ] `time == time → boolean` (NodaTime default behavior, thin wrapper)
- [ ] `time != time → boolean` (NodaTime default behavior, thin wrapper)
- [ ] `time < time → boolean` (within-day ordering, NodaTime default behavior)
- [ ] `time > time → boolean` (NodaTime default behavior, thin wrapper)
- [ ] `time <= time → boolean` (NodaTime default behavior, thin wrapper)
- [ ] `time >= time → boolean` (NodaTime default behavior, thin wrapper)
- [ ] Cross-type comparison (`time == date`, `time < instant`, etc.) is a type error
...
- [ ] `nullable` supported
- [ ] `default '...'` supported (single-quoted time)

#### Serialization

- [ ] JSON round-trip as ISO 8601 time string: `"14:30:00"`
...
- [ ] `field X as instant nullable` parses and type-checks
- [ ] `field X as instant default '2026-04-13T14:30:00Z'` parses and type-checks

#### Typed constants

- [ ] `'2026-04-13T14:30:00Z'` validates at compile time as an `instant` (content shape includes `T` and trailing `Z`)
- [ ] `'2026-04-13T14:30:00'` (no `Z`) is NOT an instant — infers as `datetime` instead; in explicit instant context produces compile error: "Instants must end with Z to indicate UTC. Use `'2026-04-13T14:30:00Z'`."
- [ ] Invalid datestamp in instant literal (e.g., `'2026-13-01T14:30:00Z'`) is a compile error
...
- [ ] `instant == instant → boolean` (NodaTime default behavior, thin wrapper)
- [ ] `instant != instant → boolean` (NodaTime default behavior, thin wrapper)
- [ ] `instant < instant → boolean` (nanoseconds on global timeline, NodaTime default behavior)
- [ ] `instant > instant → boolean` (NodaTime default behavior, thin wrapper)
- [ ] `instant <= instant → boolean` (NodaTime default behavior, thin wrapper)
- [ ] `instant >= instant → boolean` (NodaTime default behavior, thin wrapper)
- [ ] Cross-type comparison (`instant == date`, `instant < datetime`, etc.) is a type error

#### Not-supported operations (compile errors)

- [ ] `instant ± period` is a compile error: "Periods can't be added to an instant — instants use fixed-length durations. Navigate to a local type first: `FiledAt.inZone(tz).datetime ± myPeriod`, then `.inZone(tz).instant` to convert back."
...
- [ ] `instant.day` is a compile error (same)
- [ ] `instant.hour` is a compile error (same)
- [ ] `instant.minute` is a compile error (same)
- [ ] `instant.second` is a compile error (same)
- [ ] `instant.dayOfWeek` is a compile error (same)
- [ ] `instant.date` is a compile error: "An instant doesn't have a date without a timezone. Use `FiledAt.inZone(tz).date`." (Decision #22)
- [ ] `instant.time` is a compile error: skip-level accessor (Decision #22)
- [ ] `instant.datetime` is a compile error: skip-level accessor (Decision #22)

#### Accessors

- [ ] `instant.inZone(tz) → zoneddatetime` (sole accessor — no component accessors; Decision #24)

#### Constraints

- [ ] `nullable` supported
- [ ] `default '...'` supported (single-quoted instant with trailing `Z`)

#### Serialization

- [ ] JSON round-trip as ISO 8601 UTC string: `"2026-04-13T14:30:00Z"`
...
- [ ] `field X as duration` parses and type-checks
- [ ] `field X as duration nullable` parses and type-checks
- [ ] `field X as duration default 8 hours` parses and type-checks
- [ ] `field X as duration default 0 hours` parses (zero duration)
- [ ] `field X as duration default 30 minutes` parses
- [ ] `field X as duration default 3600 seconds` parses
- [ ] `field X as duration default 5 hours + 30 minutes` parses (combined defaults)

#### Typed constants

- [ ] `duration` has NO single-quoted typed constant form — postfix units only (Decision #17)
- [ ] Attempting to use a single-quoted value where a `duration` is expected is a compile error

#### Arithmetic operators

- [ ] `duration ± duration → duration` (combined elapsed time / difference)
- [ ] `duration * integer → duration` (scaling, NodaTime: `Duration * long`)
- [ ] `duration * number → duration` (scaling, NodaTime: `Duration * double`)
- [ ] `integer * duration → duration` (commutative form; Issue #10 fix)
- [ ] `number * duration → duration` (commutative form; Issue #10 fix)
...
- [ ] `duration != duration → boolean` (NodaTime default behavior, thin wrapper)
- [ ] `duration < duration → boolean` (nanoseconds, NodaTime default behavior)
- [ ] `duration > duration → boolean` (NodaTime default behavior, thin wrapper)
- [ ] `duration <= duration → boolean` (NodaTime default behavior, thin wrapper)
- [ ] `duration >= duration → boolean` (NodaTime default behavior, thin wrapper)
- [ ] Cross-type comparison (`duration == period`, `duration < instant`, etc.) is a type error
...
- [ ] `duration / decimal` is a compile error: "Durations can only be divided by whole numbers or `number`."
...
- [ ] `nullable` supported
- [ ] `default N unit` supported (e.g., `default 8 hours`, `default 30 minutes`)
- [ ] `nonnegative` supported on `duration`

#### Serialization

- [ ] JSON round-trip as ISO 8601 time-duration string: `"PT72H"`
...
- [ ] `field X as period nullable` parses and type-checks
- [ ] `field X as period default 30 days` parses
- [ ] `field X as period default 12 months` parses
- [ ] `field X as period default 2 years` parses
- [ ] `field X as period default 2 weeks` parses
- [ ] `field X as period default 2 years + 6 months` parses (combined default)
- [ ] `field X as period default 1 day + 12 hours` parses (mixed date+time default)
- [ ] `field X as period timeonly` parses (Decision #26 constraint suffix — hours/minutes/seconds only)
- [ ] `field X as period dateonly` parses (Decision #26 constraint suffix — years/months/weeks/days only)
- [ ] `field X as period timeonly default 8 hours` parses
- [ ] `field X as period dateonly default 30 days` parses
- [ ] `field X as period timeonly default 30 days` is a compile error (default violates constraint)
- [ ] `field X as period dateonly default 3 hours` is a compile error (default violates constraint)

#### Typed constants

- [ ] `period` has NO single-quoted typed constant form — postfix units only (Decision #17)
- [ ] Attempting to use a single-quoted value where a `period` is expected is a compile error

#### Arithmetic operators

- [ ] `period ± period → period` (combined calendar period / difference)
- [ ] `-period → period` (unary negation)

#### Comparison operators

- [ ] `period == period → boolean` (structural equality; Decision #15)
- [ ] `period != period → boolean` (structural equality; Decision #15)
- [ ] `1 month != 30 days` evaluates to `true` (structurally different; Decision #15)
- [ ] `7 days != 1 week` evaluates to `true` (structurally different representations)
- [ ] `1 month == 1 month` evaluates to `true` (same structure)

#### Compiler warnings (always-false / always-true literal comparisons)

- [ ] `1 month == 30 days` emits compiler warning: "This is always `false` — `1 month` and `30 days` use different parts. Period equality compares each part (years, months, days) separately."
- [ ] `1 month != 30 days` emits compiler warning: "This is always `true` — `1 month` and `30 days` use different parts."
- [ ] `7 days == 1 week` emits compiler warning (non-overlapping: days vs weeks)
- [ ] `1 month == 1 month` does NOT warn (same structure — comparison is meaningful)
- [ ] `30 days == 30 days` does NOT warn (same structure)
- [ ] Warning is emitted only when both operands are constant expressions (not field references)
- [ ] `GracePeriod == 30 days` does NOT warn (field reference — value is runtime-dependent)
- [ ] Warning severity is "warning" not "error" — code is valid, just likely surprising

#### Not-supported operations (compile errors)

- [ ] `period < period` is a compile error: "Periods can't be compared with `<`, `>`, `<=`, `>=` — only `==` and `!=`. A month isn't always the same length, so ordering isn't reliable." (Decision #14)
- [ ] `period > period` is a compile error (Decision #14)
- [ ] `period <= period` is a compile error (Decision #14)
- [ ] `period >= period` is a compile error (Decision #14)
- [ ] `period * integer` is a compile error: "Periods can't be multiplied. Write the value directly: `60 days` instead of `30 days * 2`."  
- [ ] `period ± duration` is a compile error: "Periods (days, months, years) and durations (hours, minutes, seconds) can't be mixed."
- [ ] `period == duration` is a compile error: "A period and a duration are different kinds of values and can't be compared."

#### Accessors

- [ ] `period.years → integer` (structural component, not normalized)
- [ ] `period.months → integer` (structural)
- [ ] `period.weeks → integer` (structural)
- [ ] `period.days → integer` (structural)
- [ ] `period.hours → integer` (structural)
- [ ] `period.minutes → integer` (structural)
- [ ] `period.seconds → integer` (structural)
- [ ] `14 months` has `.months == 14` and `.years == 0` (NOT auto-normalized)
- [ ] `period.hasDateComponent → boolean` (true if any of years/months/weeks/days non-zero)
- [ ] `period.hasTimeComponent → boolean` (true if any of hours/minutes/seconds non-zero)

#### Teachable error messages

- [ ] `1 month > 30 days` produces compile error: "Periods can't be compared with `<` or `>` — is 1 month greater than 30 days? It depends on which month."
- [ ] `GracePeriod * 2` produces compile error: "Periods can't be multiplied. Write the value directly, e.g. `60 days` instead of `GracePeriod * 2`."

#### Constraints

- [ ] `nullable` supported
- [ ] `default N unit` / `default N unit + M unit` supported
- [ ] `timeonly` constraint suffix (Decision #26)
- [ ] `dateonly` constraint suffix (Decision #26)

#### Serialization

- [ ] JSON round-trip as ISO 8601 period string: `"P1Y2M3D"`, `"PT5H30M"`, `"P1MT2H"` (via `PeriodPattern.Roundtrip`)
...
- [ ] `field X as timezone nullable` parses and type-checks
- [ ] `field X as timezone` with no default is valid
- [ ] `field X as timezone default 'America/New_York'` parses and type-checks
- [ ] `field X as timezone default 'UTC'` parses and type-checks
...
- [ ] No arithmetic operators defined for `timezone` — any attempt is a compile error

#### Comparison operators

- [ ] `timezone == timezone → boolean` (equality by IANA identifier)
- [ ] `timezone != timezone → boolean` (equality by IANA identifier)
- [ ] `timezone < timezone` is a compile error (no ordering)
- [ ] `timezone > timezone` is a compile error (no ordering)
- [ ] `timezone <= timezone` is a compile error (no ordering)
- [ ] `timezone >= timezone` is a compile error (no ordering)
- [ ] Cross-type comparison (`timezone == string`, etc.) is a type error

#### Accessors

- [ ] No accessors defined for `timezone`

#### Validation

- [ ] Compile-time: literal strings validated against IANA TZ database from NodaTime
- [ ] Runtime: event arguments typed `as timezone` validated at fire time

#### Constraints

- [ ] `nullable` supported
- [ ] `default '...'` supported (single-quoted IANA identifier)

#### Serialization

- [ ] JSON round-trip as IANA identifier string: `"America/Los_Angeles"`
...
- [ ] `field X as zoneddatetime nullable` parses and type-checks
- [ ] `field X as zoneddatetime` with no default is valid
- [ ] `field X as zoneddatetime default '2026-04-13T14:30:00[America/New_York]'` parses and type-checks

#### Typed constants

- [ ] `'2026-04-13T14:30:00[America/New_York]'` validates at compile time as a `zoneddatetime` (content shape: `T` + bracket-enclosed timezone `[...]`)
- [ ] `'2026-04-13T14:30:00[Not/A/Zone]'` is a compile error (invalid IANA TZ)
- [ ] Invalid datestamp in zoneddatetime literal is a compile error

#### Construction

- [ ] `zoneddatetime` is constructed via `.inZone(tz)` on `instant` or `datetime` — no standalone constructor
- [ ] `instant.inZone(tz) → zoneddatetime` (forward mediation)
- [ ] `datetime.inZone(tz) → zoneddatetime` (reverse mediation)
...
- [ ] `zoneddatetime ± period` is a compile error: "Periods can't be added directly to a zoned datetime. Navigate to `.datetime` first: `(myZdt.datetime ± myPeriod).inZone(myZdt.timezone)`."
...
- [ ] `zoneddatetime.date → date` (local calendar date in bound timezone)
- [ ] `zoneddatetime.time → time` (local time in bound timezone)
- [ ] `zoneddatetime.year → integer` (local component in bound timezone)
- [ ] `zoneddatetime.month → integer`
- [ ] `zoneddatetime.day → integer`
- [ ] `zoneddatetime.hour → integer`
- [ ] `zoneddatetime.minute → integer`
- [ ] `zoneddatetime.second → integer`
- [ ] `zoneddatetime.dayOfWeek → integer` (ISO: Monday=1, Sunday=7)

#### Teachable error messages

- [ ] `zdt + 1 month` produces: "Periods can't be added directly to a zoned datetime. Navigate to `.datetime` first: `(myZdt.datetime + 1 month).inZone(myZdt.timezone)`."
- [ ] `zdt + 5` produces: "A bare number doesn't specify a unit. Use an explicit unit like `3 days` or `1 hour`."

#### Constraints

- [ ] `nullable` supported
- [ ] `default '...'` supported (single-quoted zoneddatetime with bracket-enclosed timezone)

#### Serialization

- [ ] JSON round-trip as two-property JSON: `{ "instant": "2026-04-13T14:30:00Z", "timezone": "America/New_York" }`
...
- [ ] `field X as datetime nullable` parses and type-checks
- [ ] `field X as datetime default '2026-04-13T09:00:00'` parses and type-checks

#### Typed constants

- [ ] `'2026-04-13T09:00:00'` validates at compile time as a `datetime` (content shape: `T` present, no trailing `Z`, no `[...]`)
- [ ] `'2026-04-13T09:00:00Z'` is NOT a `datetime` — infers as `instant` (trailing `Z`)
- [ ] Invalid datestamp in datetime literal (e.g., `'2026-13-01T09:00:00'`) is a compile error
...
- [ ] `datetime == datetime → boolean` (NodaTime default behavior, thin wrapper)
- [ ] `datetime != datetime → boolean` (NodaTime default behavior, thin wrapper)
- [ ] `datetime < datetime → boolean` (same-calendar ordering, NodaTime default behavior)
- [ ] `datetime > datetime → boolean` (NodaTime default behavior, thin wrapper)
- [ ] `datetime <= datetime → boolean` (NodaTime default behavior, thin wrapper)
- [ ] `datetime >= datetime → boolean` (NodaTime default behavior, thin wrapper)
- [ ] Cross-type comparison (`datetime == instant`, `datetime < date`, etc.) is a type error
...
- [ ] `datetime ± durationField` (field typed as `duration`) is a compile error (Decision #27): "`Elapsed` is a duration. Datetimes need calendar units. Use a `period` field instead, or add units directly: `AppointmentTime + 2 hours`."
...
- [ ] `datetime.dayOfWeek → integer` (ISO: Monday=1, Sunday=7)

#### Navigation via `.inZone(tz)`

- [ ] `datetime.inZone(tz) → zoneddatetime` (anchor local reading to timeline in timezone)
- [ ] `datetime.inZone(tz).instant → instant` (reverse navigation — replaces eliminated `pin`; Decision #23)

#### Constraints

- [ ] `nullable` supported
- [ ] `default '...'` supported (single-quoted datetime)

#### Serialization

- [ ] JSON round-trip as ISO 8601 without timezone: `"2026-04-13T14:30:00"`

---

### Cross-cutting: Postfix unit literals (Decision #17)

#### Bare postfix form

- [ ] `<integer-literal> <unit-keyword>` parses as a postfix unit expression
- [ ] `30 days` parses
- [ ] `72 hours` parses
- [ ] `3 months` parses
- [ ] `1 year` parses
...
- [ ] `( <expression> ) <unit-keyword>` parses as a postfix unit expression
- [ ] `(GraceDays) days` parses
- [ ] `(X + 5) hours` parses
- [ ] `(TermMonths + 6) months` parses
- [ ] `(SlaHours * 2) hours` parses

#### Negative paren-postfix expressions

- [ ] `(-5) days` is valid — produces a negative period (`Period.FromDays(-5)`)
- [ ] `date + (-5) days` → same result as `date - 5 days`
- [ ] `(-3) months` is valid — produces a negative period (`Period.FromMonths(-3)`)
- [ ] `(-2) hours` is valid in duration context — produces a negative duration (`Duration.FromHours(-2)`)
- [ ] Negative paren-postfix in defaults: `field X as period default (-30) days` is valid

#### Integer requirement (Decision #28)

- [ ] `(0.5) days` is a compile error: "Unit values must be whole numbers. Use `12 hours` for half a day."
- [ ] `(2.5) months` is a compile error (same)
- [ ] `(SomeDecimalField) hours` where field is `decimal` is a compile error
- [ ] `(SomeNumberField) days` where field is `number` is a compile error
- [ ] Integer literal in bare postfix (e.g., `30 days`) implicitly satisfies the integer requirement

#### Combination via `+`

- [ ] `2 years + 6 months` combines via standard `+` operator
- [ ] `2 years + 6 months + 15 days` chains correctly
- [ ] `5 hours + 30 minutes + 15 seconds` chains correctly
- [ ] `1 day + 12 hours` mixed date+time components combine
- [ ] `1 year + 3 months + 15 days` chains correctly

#### Juxtaposition rejection

- [ ] `2 years 6 months` (without `+`) is a compile error: "Unit values must be combined with `+`. Use `2 years + 6 months`."
- [ ] `30 days 12 hours` (without `+`) is a compile error (same)

#### Variable bare postfix rejection

- [ ] `GraceDays days` (variable without parens) is a compile error: "Wrap field references in parentheses when used with units. Use `(GraceDays) days`."

#### Ambiguous standalone rejection

- [ ] `set X = 3 days` with no typed context is a compile error: "Can't determine the type of `3 days` without context. Use it in an expression like `DueDate + 3 days` so the type is clear."
- [ ] `set X = 72 hours` with no typed context is a compile error (same)

#### Left-associative `+` subtlety

- [ ] `date + 1 month + 1 month` applies sequential truncation (Jan 31 → Feb 28 → Mar 28)
- [ ] `date + (1 month + 1 month)` builds period first then applies once (Jan 31 → Mar 31)

#### Chained arithmetic (mixed `+` and `-`)

- [ ] `date + 3 months - 15 days → date` (left-to-right: `(date + 3 months) - 15 days`)
- [ ] `time + 3 hours - 30 minutes → time` (left-to-right, with potential midnight wrapping)
- [ ] `instant + 2 hours + 30 minutes - 15 seconds → instant` (3-op chain)
- [ ] `datetime + 1 year - 2 months + 15 days → datetime` (4-op chain, left-to-right)

#### Unit keywords not consumed as identifiers

- [ ] `days`, `months`, `years`, `weeks`, `hours`, `minutes`, `seconds` in postfix position are parsed as unit keywords, not identifiers

---

### Cross-cutting: Single-quoted typed constants (Decision #18)

#### Shape-based type inference

- [ ] `'2026-01-15'` infers as `date` (shape: `YYYY-MM-DD`, digits + hyphens, no `T`)
- [ ] `'14:30:00'` infers as `time` (shape: `HH:mm:ss`, digits + colons, no hyphen)
- [ ] `'14:30'` infers as `time` (seconds optional)
- [ ] `'2026-01-15T14:30:00'` infers as `datetime` (shape: `T` present, no `Z`, no `[`)
- [ ] `'2026-01-15T14:30:00Z'` infers as `instant` (shape: `T` + trailing `Z`)
- [ ] `'2026-01-15T14:30:00[America/New_York]'` infers as `zoneddatetime` (shape: `T` + `[zone]`)
- [ ] `'America/New_York'` infers as `timezone` (shape: IANA `Word/Word` pattern)

#### Admission rule enforcement

- [ ] No shape matches two types — each single-quote inhabitant is unambiguous
- [ ] Content inside single quotes is a constant pattern, not an expression — `'2024-01-15'` does NOT resolve as subtraction

#### Delimiter distinction

- [ ] `"2026-01-15"` remains a `string` (double-quoted)
- [ ] `'2026-01-15'` is a `date` (single-quoted)
- [ ] Using the wrong delimiter produces a teachable error

#### IntelliSense

- [ ] `'` triggers typed constant completions with format-aware ghost text
- [ ] After `'2024-`, show `-MM-DD` placeholder
- [ ] After `'14:`, show `mm:ss` placeholder
- [ ] `'` in timezone context triggers IANA timezone completion from `DateTimeZoneProviders.Tzdb.Ids` with UTC offset descriptions
- [ ] After `default ` on a `date` field, completion offers `default '2026-01-15'`
- [ ] After `default ` on a `time` field, completion offers `default '09:00:00'`
- [ ] After `default ` on an `instant` field, completion offers `default '2026-04-13T14:30:00Z'`
- [ ] After `default ` on a `datetime` field, completion offers `default '2026-04-13T09:00:00'`
- [ ] After `default ` on a `duration` field, completion offers a postfix-unit snippet such as `default 8 hours`
- [ ] After `default ` on a `period` field, completion offers a postfix-unit snippet such as `default 30 days`
- [ ] After `default ` on a `timezone` field, completion offers `default 'America/New_York'`
- [ ] After `default ` on a `zoneddatetime` field, completion offers `default '2026-04-13T14:30:00[America/New_York]'`
- [ ] Default-position completions are normalized by declared field type rather than one generic temporal snippet

#### Dot-accessor IntelliSense

- [ ] After `myInstant.`, completion offers `.inZone(tz)` as the sole entry
- [ ] `.inZone(tz)` completion detail says: "An instant is a point on the timeline with no timezone. Use `.inZone(tz)` to see it as a local date and time."
- [ ] After `.inZone(tz).`, completions offer `.date`, `.time`, `.datetime`, `.instant`, `.timezone`, `.year`, `.month`, `.day`, `.hour`, `.minute`, `.second`, `.dayOfWeek`
- [ ] After `myDate.`, completions do NOT offer `.inZone(tz)` (dates have no timezone mediation)
- [ ] After `myZdt.`, completions offer `.instant`, `.timezone`, `.datetime`, `.date`, `.time` and component accessors
- [ ] Dot-accessor completions are type-aware — only valid accessors for the left operand's type appear

#### Hover content (invariant culture, deterministic)

- [ ] `'2024-01-15'` hover shows "date: January 15, 2024 (Monday)"
...
### Cross-cutting: Context-dependent type resolution (Decision #17)
...
#### `months` and `years` — always `period`
...
### Cross-cutting: Dot-chain navigation

- [ ] `instant.inZone(tz).date` works (instant → zoneddatetime → date)
- [ ] `instant.inZone(tz).time` works (instant → zoneddatetime → time)
- [ ] `instant.inZone(tz).datetime` works (instant → zoneddatetime → datetime)
- [ ] `instant.inZone(tz).date.year` works (multi-step: instant → zdt → date → integer)
- [ ] `instant.inZone(tz).time.hour` works (multi-step: instant → zdt → time → integer)
- [ ] `instant.inZone(tz).year` works (shortcut: zoneddatetime provides component accessors directly)
- [ ] `datetime.inZone(tz).instant` works (reverse navigation: datetime → zdt → instant; Decision #23)
- [ ] `zoneddatetime.datetime.date` works (step-by-step zdt → datetime → date)
- [ ] `zoneddatetime.date` works (direct: zdt provides `.date` directly)
- [ ] Results from dot-chain navigation support all operations of their produced type (e.g., `instant.inZone(tz).date + 30 days` works)

---

### Cross-cutting: Strict type hierarchy — no skip-level accessors (Decision #22)

- [ ] `instant.date` is a compile error (skips timezone mediation)
- [ ] `instant.time` is a compile error (skips timezone mediation)
- [ ] `instant.datetime` is a compile error (skips timezone mediation)
- [ ] `instant.year` is a compile error (skips timezone mediation)
- [ ] `instant.month` is a compile error (same)
- [ ] `instant.day` is a compile error (same)
- [ ] `instant.dayOfWeek` is a compile error (same)
- [ ] `instant.hour` is a compile error (same)
- [ ] `instant.minute` is a compile error (same)
- [ ] `instant.second` is a compile error (same)
- [ ] `datetime.instant` is a compile error (must go through `.inZone(tz)`)
- [ ] `date.inZone(tz)` is a compile error: "A date has no time component — `.inZone(tz)` requires both date and time. Combine with a time first: `myDate + '00:00:00'` to create a datetime, then `.inZone(tz)`."
- [ ] `time.inZone(tz)` is a compile error: "A time has no date component — `.inZone(tz)` requires both date and time."
- [ ] `duration.inZone(tz)` is a compile error: "`.inZone(tz)` applies to instants and datetimes, not quantities."
- [ ] `period.inZone(tz)` is a compile error: "`.inZone(tz)` applies to instants and datetimes, not quantities."

---

### Cross-cutting: Duration bridging — `time + duration` (Decision #16)
...
### Cross-cutting: `date + time → datetime` (Decision #25)

- [ ] `date + time → datetime` (`LocalDate + LocalTime → LocalDateTime`)
- [ ] `time + date → datetime` (commutative — same result)
- [ ] `'2026-06-01' + '09:00:00' → datetime` (literals in both positions)
- [ ] `DateField + TimeField → datetime` (field references in both positions)
- [ ] `date + time` result supports all `datetime` operations (`.year`, `.inZone(tz)`, `+ period`, etc.)
- [ ] No factory function (`datetime(date, time)`) exists — `date + time` is the sole construction path

---

### Cross-cutting: Period component constraint enforcement (Decision #26)

#### Constraint declaration

- [ ] `field X as period timeonly` — accepts hours/minutes/seconds only
- [ ] `field X as period dateonly` — accepts years/months/weeks/days only
- [ ] `event Foo with Delay as period timeonly` — event arg with constraint
- [ ] `event Bar with Term as period dateonly` — event arg with constraint
- [ ] `period` (unconstrained) field — accepts any components

#### Enforcement at `time` arithmetic sites

- [ ] `time ± period timeonly → time` — OK
- [ ] `time ± period` (unconstrained field/arg) → compile error
- [ ] `time ± period dateonly` → compile error (date-only period can't be added to a time)
...
- [ ] `date ± period timeonly` → compile error (time-only period can't be added to a date)
...
- [ ] `datetime ± period` (unconstrained) → OK
- [ ] `datetime ± period timeonly` → OK
- [ ] `datetime ± period dateonly` → OK

#### Constraint arithmetic closure

- [ ] `period timeonly + period timeonly → period timeonly`
- [ ] `period dateonly + period dateonly → period dateonly`
- [ ] `period timeonly + period dateonly → period` (unconstrained mixed)
- [ ] `period dateonly + period timeonly → period` (unconstrained mixed)
- [ ] `time - time → period` result is implicitly time-only (no constraint annotation needed)
- [ ] `date - date → period` result is implicitly date-only (no constraint annotation needed)

#### Mixed-constraint result usability

- [ ] `(timeonlyPeriod + dateonlyPeriod)` result is usable with `datetime ± period` → OK (datetime accepts all components)
- [ ] `(timeonlyPeriod + dateonlyPeriod)` result with `date ± period` → compile error (unconstrained period on date)
- [ ] `(timeonlyPeriod + dateonlyPeriod)` result with `time ± period` → compile error (unconstrained period on time)

#### API boundary validation

- [ ] Event args declared `period timeonly` are validated at fire time — input with date components is rejected
- [ ] Event args declared `period dateonly` are validated at fire time — input with time components is rejected
- [ ] Validation uses `Period.HasDateComponent` / `Period.HasTimeComponent` from NodaTime
...
### Cross-cutting: Nullable and default behavior

- [ ] All 8 temporal types support `nullable`
- [ ] All temporal types follow existing null propagation rules

#### Null propagation through temporal arithmetic

- [ ] `nullableDate ± 30 days` → null (null propagation through period arithmetic)
- [ ] `nullableInstant - otherInstant` → null (null propagation through subtraction)
- [ ] `nullableDuration * 2` → null (null propagation through scaling)

#### Null propagation through accessors

- [ ] `nullableDate.year` → null (null propagation through accessor)
- [ ] `nullableDuration.totalHours` → null (null propagation through accessor)
- [ ] `nullablePeriod.hasDateComponent` → null (null propagation through introspection)

#### Null propagation through `.inZone(tz)` mediation

- [ ] `nullableInstant.inZone(tz)` → null (null propagation through `.inZone(tz)`)
- [ ] `myInstant.inZone(nullTimezone)` → null (null propagation through timezone argument)

#### Null propagation through composition

- [ ] `nullableDate + '14:30:00'` → null (null propagation through `date + time → datetime`)

#### Default values
- [ ] `date` default via single-quoted literal: `default '2026-06-01'`
- [ ] `time` default via single-quoted literal: `default '09:00:00'`
- [ ] `instant` default via single-quoted literal: `default '2026-04-13T14:30:00Z'`
- [ ] `duration` default via postfix unit: `default 8 hours`
- [ ] `period` default via postfix unit: `default 30 days` / `default 12 months` / `default 2 years + 6 months`
- [ ] `timezone` default via single-quoted literal: `default 'America/New_York'`
- [ ] `zoneddatetime` default via single-quoted literal: `default '2026-04-13T14:30:00[America/New_York]'`
- [ ] `datetime` default via single-quoted literal: `default '2026-04-13T09:00:00'`
...
- [ ] `event Foo with T as time` fired with `"25:00:00"` → rejected at fire time (invalid hour)
- [ ] `event Foo with DT as datetime` fired with `"2026-04-13"` (no time component) → rejected at fire time
...
### Cross-cutting: Zero-value identity edge cases

- [ ] `date ± 0 days` → same date (identity)
- [ ] `time ± 0 hours` → same time (identity)
- [ ] `instant ± 0 seconds` → same instant (identity)
- [ ] `duration ± 0 hours` → same duration (identity / zero duration)
- [ ] `0 days` and `0 months` are structurally different periods (`Period.FromDays(0)` vs. `Period.FromMonths(0)` — NodaTime structural equality distinguishes them)
- [ ] `period == period` uses structural equality: `0 days == 0 days` → true, `0 days == 0 months` → false

---

### Cross-cutting: Numeric constraint rejection on temporal types

- [ ] Numeric constraints (`nonnegative`, `min`, `max`, `maxplaces`, `minlength`, `maxlength`) on `date` are compile errors
- [ ] Numeric constraints on `time` are compile errors
- [ ] Numeric constraints on `instant` are compile errors
- [ ] Numeric constraints on `datetime` are compile errors
...
### Cross-cutting: Bare numeric arithmetic rejection on temporal point types

- [ ] All temporal point types (`date`, `time`, `instant`, `datetime`, `zoneddatetime`) reject arithmetic with bare `integer`, `number`, and `decimal` — a unit keyword is always required
- [ ] `time ± integer` → compile error
- [ ] `time ± number` → compile error
- [ ] `time ± decimal` → compile error
- [ ] `instant ± number` → compile error
- [ ] `instant ± decimal` → compile error
- [ ] `datetime ± number` → compile error
- [ ] `datetime ± decimal` → compile error
- [ ] `zoneddatetime ± number` → compile error
- [ ] `zoneddatetime ± decimal` → compile error
- [ ] `period ± integer` → compile error (bare integer has no unit)
...
- [ ] `duration * duration` → compile error (can't multiply two durations together)
- [ ] `duration * decimal` → compile error (durations can only be multiplied by whole numbers)
- [ ] `duration / decimal` → compile error (durations can only be divided by whole numbers — use `number`)
- [ ] `date - time` → compile error (subtraction doesn't reverse composition; use accessors to decompose a `datetime`)
- [ ] All "Not supported" table rows produce compile errors, not runtime exceptions

---

### Cross-cutting: Teachable error messages

- [ ] Every compile error listed in the proposal's "Teachable error messages" tables produces the specified (or equivalent) message
- [ ] Error messages suggest the correct alternative (e.g., "Use `DueDate + 2 days`" instead of just "type error")
- [ ] Error messages name the types involved (e.g., "Periods and instants can't be mixed")
- [ ] Date format error suggests ISO 8601: "expected ISO 8601 (YYYY-MM-DD)"
- [ ] Instant missing `Z` error explains the difference: "Without Z, this is a `datetime`, not an `instant`."
- [ ] Skip-level accessor errors suggest the correct dot chain: "Use `FiledAt.inZone(tz).date.year`"
- [ ] Period ordering error explains why: "depends on which month"
- [ ] Postfix juxtaposition error directs to `+`: "Use `2 years + 6 months`"
- [ ] Variable bare postfix error directs to paren form: "Use `(GraceDays) days`"

#### Field-type-aware diagnostics

- [ ] When a cross-type arithmetic error involves a field reference, the error names the field and its declared type
- [ ] `DueDate + Elapsed` (where `Elapsed` is `duration`) produces: "`Elapsed` is a duration (hours/minutes/seconds). Dates need calendar units like `days` or `months`."
- [ ] `FiledAt + GracePeriod` (where `GracePeriod` is `period`) produces: "`GracePeriod` is a period (days/months/years). Instants need fixed-length units. Convert to a date first: `FiledAt.inZone(tz).date + GracePeriod`."
...
### Cross-cutting: Parser / tokenizer infrastructure

- [ ] `date`, `time`, `instant`, `duration`, `period`, `timezone`, `zoneddatetime`, `datetime` recognized as type keywords
- [ ] `days`, `months`, `years`, `weeks`, `hours`, `minutes`, `seconds` recognized as unit keywords in postfix position
- [ ] `inZone` recognized as dot-accessor keyword for timezone mediation
- [ ] New token type `TypedConstantLiteral` matches `'[^']*'` as a single token
- [ ] `PostfixUnitExprBare` (`<integer-literal> <unit-keyword>`) precedes `NumberAtom` in atom chain
- [ ] `PostfixUnitExprParen` (`( <expression> ) <unit-keyword>`) precedes `ParenExpr` in atom chain with `.Try()` backtracking
- [ ] Unit keywords in postfix position are NOT consumed as identifiers

---

### Cross-cutting: TextMate grammar sync

- [ ] All 8 type keywords added to `typeKeywords`
- [ ] All 7 unit keywords added to appropriate keyword group
- [ ] Single-quoted literal pattern (`'[^']*'`) added as typed constant token
- [ ] `inZone` added as dot-accessor keyword

---

### Cross-cutting: Language server sync

- [ ] Completions for all 8 type keywords in field and event arg declarations
- [ ] Hover info for all 8 types (see table below)
- [ ] Diagnostics for all type errors specified in this proposal
- [ ] Single-quote trigger for typed constant completions
- [ ] Period-specific diagnostics (ordering rejection, scaling rejection, cross-domain rejection)
- [ ] Sub-day bridging documentation in hover for `time + duration`

#### Type keyword hover content

| Type | Hover text |
|------|-----------|
| `date` | A calendar date with no time-of-day component. Combine with a time using `date + time` to produce a datetime. |
| `time` | A time of day with no date component. Combine with a date using `date + time` to produce a datetime. |
| `datetime` | A date and time with no timezone information. Use `.inZone(tz)` to anchor it to a specific timezone. |
| `instant` | An exact, unambiguous point on the UTC timeline. Use `.inZone(tz)` to access date or time components in a specific timezone. |
| `duration` | A fixed elapsed time measured in hours, minutes, and seconds. Use for precise timeline arithmetic — not for calendar concepts like days or months. |
| `period` | A variable calendar quantity measured in days, months, or years. Supports equality checks only — periods cannot be ordered because 1 month is not a fixed length. |
| `timezone` | An IANA timezone identifier (e.g. `America/New_York`). Used with `.inZone(tz)` to anchor an instant or datetime to a real-world location. |
| `zoneddatetime` | A date and time anchored to a specific timezone. Constructed via `.inZone(tz)` on an instant or datetime — captures both the moment and where it's observed. |

---

### Cross-cutting: MCP tool sync

- [ ] `precept_language` lists all 8 types, postfix unit system, typed constant delimiter syntax
- [ ] `precept_compile` / `precept_fire` / `precept_inspect` / `precept_update` handle all 8 types
- [ ] `period` serialized as ISO 8601 string (`P1Y2M3D`) in MCP responses
- [ ] `duration` serialized as ISO 8601 time-duration (`PT72H`) in MCP responses
- [ ] `date`, `time`, `instant`, `datetime` serialized as ISO 8601 strings
- [ ] `timezone` serialized as IANA identifier string
- [ ] `zoneddatetime` serialized as two-property JSON `{ "instant": "..Z", "timezone": "..." }`
