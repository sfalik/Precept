...
- **Prevention, not detection.** Invalid entity configurations — combinations of lifecycle position and field values that violate declared rules — cannot exist. They are structurally prevented before any change is committed, not caught after the fact. The contract prevents them. This commitment extends to the expression layer itself. Any expression that could fault at runtime — division by zero, overflow, empty collection access — must be provably safe at compile time, or the compiler must require the author to supply constraints that make safety provable. A precept that compiles without diagnostics has no unproven evaluation faults.
