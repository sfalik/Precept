...
**3. Domain integrity terminology.** "Domain integrity engine" is the correct product label (DDD sense — the business problem space, not the relational/Date sense). "Governed integrity" is the load-bearing technical term. A future revision to `philosophy.md` should anchor which sense of "domain" is in use. See `research/philosophy/domain-integrity-formal-concept.md`.
