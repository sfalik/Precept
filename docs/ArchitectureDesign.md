...
**Single-pass.** Both the type checker and the evaluator operate in a single forward pass. No fixpoint computation, no iterative widening, no reconverging flow. This is not an engineering tradeoff — it is a structural consequence of Precept's flat execution model: no loops, no control-flow branches, no reconverging paths in the DSL.
...
1. **The compile-time gateway is the prevention mechanism.** `PreceptEngine` is immutable and can only be constructed from a validated definition. `PreceptCompiler` enforces this — there is no constructor bypass, no partial-engine, no "compile later" mode. This is the architectural realization of "invalid configurations cannot exist": the run-time phase never encounters an unvalidated definition. The compile-time gate is where prevention happens. *(Philosophy: "Prevention, not detection.")*
...
The following items emerged from external architecture research and are tracked here for consideration in future design work. They do not reflect planned changes — they are open questions, precision debts, and evolution triggers that should inform decisions if the relevant areas are revisited.

**1. Construction invariant precision.** The current wording in Principles 1 and 2 characterizes the guarantee as "invalid configurations cannot exist." The more precise characterization is: "`PreceptEngine` cannot be constructed from an invalid definition" — the guarantee is a *construction invariant*, not a type-level impossibility. The soundness of the compile-time gate rests on five explicit conditions: (1) opaque construction through `PreceptCompiler`, (2) total validation in `TypeChecker.Check` before any engine is produced, (3) immutability of `PreceptEngine` after construction, (4) phase segregation — no shared mutable state between phases, and (5) one-file completeness — no external definition sources. When these five conditions hold, the construction invariant holds. This precision matters if the engine ever exposes a serialization surface or if .NET reflection becomes a supported integration pattern. See `research/architecture/compile-time-gate-pattern.md`.

**2. Type checker phase formalization and incremental compilation.** The `ComputedFieldOrder` pass involves two micro-passes: a topological sort (first traversal) and a type check (second traversal). The current "single-pass type checker" characterization in `## Cross-Cutting Properties` elides this. As the type checker grows, formalizing the phase structure (topological sort → interval injection → type check → proof assessment) will make the ordering constraints explicit and support incremental re-checking. For incremental compilation, two cheap near-term wins are available in the language server: `TextDocumentSyncKind.Incremental` (reduces parse work on each keystroke) and a compile debounce (avoids redundant full checks mid-edit). K2/FIR-style phase formalization is the right direction when `.precept` files routinely exceed ~1,500 lines or when IDE latency becomes noticeable. See `research/architecture/incremental-compilation-patterns.md` and `research/architecture/single-pass-type-checking.md`.

**3. Parser combinator evolution trigger.** The recursive-descent parser is the correct choice for the current grammar. The specific evolution trigger to watch for is *angle-bracket type syntax in expression position* — if a future language version uses angle-bracket type expressions as expression components rather than only in field declarations, the `<` token becomes ambiguous with comparison operators. At that point, parser-combinator or PEG-style parsing is worth evaluating. As long as type annotations remain declaration-only, the current parser handles ambiguity resolution without lookahead. See `research/language/parser-combinator-scalability.md`.

**4. Domain integrity terminology.** "Domain integrity engine" is the correct product label — grounded in DDD's "domain" as the business problem space, not the relational/database sense. The load-bearing technical term is "governed integrity": the engine enforces a formally authored definition's rules across the full lifecycle. A future revision of `philosophy.md` should add one sentence anchoring which sense of "domain" Precept uses, to prevent the DDD sense from being conflated with the relational/Date sense when reviewing or extending the architecture. See `research/architecture/domain-integrity-terminology.md`.

**5. Hierarchical state modeling.** Precept's first-match flat routing is formally equivalent to the flat-state fragment of SCXML DocumentOrder. Adding hierarchical states (parent/child, parallel regions) is not an additive feature — it is a ground-up redesign. Hierarchical state semantics require compound state resolution, orthogonal-region execution order, inner-first event processing, history state tracking, and a multi-pass transition algorithm. These additions touch the parser, type checker, engine, evaluator, proof engine, and all MCP/language-server outputs simultaneously. Tracked as a long-range design consideration. See `research/architecture/hierarchical-state-analysis.md`.
