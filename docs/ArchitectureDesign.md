...
**3. Domain integrity terminology.** "Domain integrity engine" is the correct product label (DDD sense — the business problem space, not the relational/Date sense). "Governed integrity" is the load-bearing technical term. A future revision to `philosophy.md` should anchor which sense of "domain" is in use. See `research/architecture/domain-integrity-terminology.md`.

**4. Hierarchical state modeling.** Precept's first-match flat routing is formally equivalent to the flat-state fragment of SCXML DocumentOrder. Adding hierarchical states (parent/child, parallel regions) is a ground-up redesign — it touches the parser, type checker, engine, evaluator, proof engine, and all tooling outputs simultaneously. Tracked as a long-range design question. See `research/architecture/hierarchical-state-analysis.md`.
