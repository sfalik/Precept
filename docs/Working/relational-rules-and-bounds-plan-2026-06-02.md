...
| 2c-i | Relational narrowing **core** (field-to-field `>=`/`<=` → provable interval; R-NARROW-*, single-pass depth-1) | Decision 2 (=B) + Decision 4 | none (closure-depth = falsifier-gated knob) | L (4–6d) | Next |
| 2c-ii | Field-ref modifier bounds enforced (`min Floor` desugars + proof-participates; target author message) | BUG-020 enforcement half + Decisions 5/6/7 | none (attribution-confirm at build) | M–L (3–5d) | Stub (gates on 2c-i) |
...
- **2c — next** — relational narrowing (D2/D4) + field-ref bounds (5/6/7) + BUG-021 (flag-fold into the set-action path, gated on the narrowing reach for the ~55 samples). The XL centerpiece, **promoted 2026-06-03 to three coupled sub-slices in dependency order**: 2c-i relational narrowing core (heavyweight, the foundation) → 2c-ii field-ref modifier bounds (stub, gates on 2c-i) → 2c-iii BUG-021 set-action lower bound (stub, gates on 2c-i). See § Heavyweight phase block — Slice 2c.
