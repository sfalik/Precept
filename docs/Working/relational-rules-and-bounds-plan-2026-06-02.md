...
### Discovered during execution (surfaced, not resolved)

- **D20 bare-identifier scoping — spec/impl drift (language-surface; surfaced for owner).** `ResolveIdentifier` (TypeChecker, "D20") resolves a **bare** identifier to an in-scope **event arg before a same-named field** (arg shadows field). spec §3.5 line 1363 ("event args are accessed via dotted notation `EventName.ArgName`") reads as if a bare identifier is the field — internally inconsistent with D20 and the implementation. This is *sound* as implemented (bare binds to the arg at both compile and runtime — verified: a field `Position min 5` shadowed by an unconstrained arg `Position` correctly emits `DivisionByZero` on `100 / Position`), but it's a potential authoring footgun and the spec is ambiguous. Surfaced per the consultation gate; not blocking 2a. Owner decision: confirm D20 intended (→ clarify §3.5 dotted-access wording) vs. bare=field intended (→ `ResolveIdentifier` priority is the bug, a type-checker scoping change).
