...
**Status**: Draft — 2026-07-19 (rev 3: destined for canon per owner ruling; exact-power iff contract + per-cell respellability added per owner ruling; rev 2 generalized per premise reviews; structure only, population pending)
...
2. **Machines verify every cell** — the day-one validator (Storage, above): disposition totality, citations present and resolving, axis-value validity, near-miss pairing, respellability verdicts present, WP recomputation where mechanized. No cell ships unchecked.
