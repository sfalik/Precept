...
Decision procedure — class (c), two licensed derivations: normal-form match of the WP against the row's guard conjuncts (a finite set); failing that, collect the guard's per-term bound conjuncts (`term op constant`) over the WP's terms and run the same exact-decimal interval combination as class (b) — a term with no bound conjunct, or a combined bound exceeding the rule bound, fails the derivation. Class (b): exact-decimal upper-bound sum over the args' declared `max` bounds; an arg with no upper bound, or a computed sum exceeding the rule bound, fails the class. All derivations failing rejects, naming both classes.
...
Decision procedure — shape-match the RHS as written-field-minus-term (one syntactic pattern); look up a nonnegativity fact for the subtracted term in its declared modifiers (a finite set — the modifier→ProofSatisfaction table, `proof-engine.md` § Strategy 2) and the rule's own bound in the pre-state hypothesis; either lookup failing rejects.
