...
research-status:
  external-engagement: partial. TypeScript and Kotlin precedent claims are well-established and verifiable from publicly available language documentation. Rust precedent claim in Decision 5 v2 is precision-issue — `mut` appears at pattern bindings, function parameters, and reference types but NOT at struct field declarations specifically. SQL `GRANT UPDATE` is a conceptual analogy (privilege grant DDL) rather than a same-keyword-at-multiple-positions syntactic precedent. The "every comparable language uses one keyword across positions" claim is correct for two of four comparators (TypeScript, Kotlin); narrower for Rust; loose for SQL.
  next-step: full comparator survey in `research/language/expressiveness/access-modifier-keyword-unification.md` covering TypeScript, Kotlin, Rust, Swift, C#, Java, F#, Scala — with verbatim excerpts and stable identifiers — BEFORE the design re-Locks for Phase 5 implementation. Comparator survey is Phase 8 → Phase 10 prerequisite work for this design's irreversible Decision 5.
---
...
- **Precedent — partial; precision issues surfaced by Phase 8 retrofit**: The decision-v2 claim was *"Every comparable language with a layered access-capability model uses one keyword across positions"* with four named comparators (TypeScript `readonly`, Rust `mut`, Kotlin `val`/`var`, SQL `GRANT UPDATE`). On retrofit, two of four hold; two need revision:
  - **TypeScript `readonly` — verified.** The TypeScript handbook (`typescriptlang.org/docs/handbook/2/objects.html` § Readonly Properties) shows `readonly` used at interface property declarations, class property declarations, mapped types (`Readonly<T> = { readonly [P in keyof T]: T[P] }`), and `readonly` tuple/array types. Same keyword across positions. **Verbatim excerpt pending source-fetch; provisional.**
  - **Kotlin `val`/`var` — verified.** The Kotlin reference (`kotlinlang.org/docs/properties.html` § Declaring properties) shows `val`/`var` used at class property declarations and at local variable declarations. Same keyword across positions. **Verbatim excerpt pending source-fetch; provisional.**
  - **Rust `mut` — precision issue.** `mut` appears at pattern bindings (`let mut x = 5`), function parameters (`fn f(mut x: T)`), method receivers (`fn f(&mut self)`), and reference types (`&mut T`). It does NOT appear at struct field declarations — Rust struct fields are not individually marked `mut`; mutability of struct fields follows the binding mutability of the struct instance (`let mut foo = Foo { x: 1 }; foo.x = 2`). The decision's framing ("Rust `mut` appears at field declarations and in patterns/bindings — same word") is **not accurate**. The narrower true claim: Rust uses one `mut` keyword across bindings, parameters, methods, and reference types — but not as a field-declaration precedent. **Decision 5's Rust precedent claim should be removed or re-framed.**
  - **SQL `GRANT UPDATE` — analogy, not syntactic precedent.** SQL privilege grants are DDL separate from table-creation DDL; there's no "same keyword at multiple positions in the schema language" pattern. The comparison is conceptual ("write capability at scope") rather than the same-keyword-at-multiple-positions precedent the decision claimed. **Should be removed from the precedent leg or explicitly labeled as conceptual analogy.**
  - **Net effect on the decision's rationale**: The "every comparable language uses one keyword" claim is overstated. The accurate claim is "TypeScript and Kotlin both use one keyword across property-declaration and other positions." That precedent supports unification, just narrower than originally stated. **The decision is still defensible**, but on narrower grounds.
