...
| [phase8-slice2-code-mediation-2026-06-01.md](phase8-slice2-code-mediation-2026-06-01.md) | Promoted 2026-06-04 (canon synced in ship commit `59fe2666`) | `compiler/diagnostic-system.md § Emission shapes` + `§ [StaticallyPreventable]-derived fault map`; `compiler/proof-engine.md § ProofLedger Construction` (literal-vs-catalog-mediated code selection; `CreateDiagnostic` reads `ProofRequirementMeta.DiagnosticCode`; bijective fault rows reflected from the attribute) |
| [phase8-value-level-obligation-ownership-2026-06-01.md](phase8-value-level-obligation-ownership-2026-06-01.md) | Promoted 2026-06-04 (canon synced in ship commits `ecce5d51`, `9820b8c1`) | `compiler/proof-engine.md § Strategy 8/9/10` + `§ Computed-field bound containment`; `compiler/diagnostic-system.md § DiagnosticStage`; `tooling/mcp.md § Per-stage counts` (numeric default→OutOfRange, length/count default stamping, count-prover revival, BUG-017 close). ⚠️ Carries a Correction header — original P11-completeness claims were a rewrite, not load-bearing |
