---
status: Promoted 2026-05-27 — implemented in commits `a6c35e36`, `ffb7288b`, `c16be77b`, `42cee783` (Phase 5 W-C); soundness fixes added in commits `11599944` (Slice 5 PRE0159 + reachability) and `cf8ffcd1` (Slice 6 source-order + wildcard-row gate + `when`-guard mutual-exclusion). Canonical content lives in `docs/compiler/proof-engine.md` (satisfiability strategies; cross-row interval composition; `ProofForwardingFact.UnreachableRowFact` variant), `docs/compiler/diagnostic-system.md § PRE0153/PRE0154/PRE0155/PRE0156/PRE0159` (UnsatisfiableGuard, ContradictoryRule, VacuousRule, TautologicalGuard, UnsatisfiableRule), `docs/language/precept-language-spec.md § 0.6` (F-LANG-SPEC-02/03/04/05/12 obligations moved from Specification-only to Implemented). This archive retains the four-leg rationale and the inline GHC / CUE / Dafny / Liquid Haskell comparator survey for future audit reference. Original Locked history: Locked 2026-05-26 — refreshed 2026-05-26 (precept-reviewer remediation: NumericInterval.IsEmpty already exists at NumericInterval.cs:11 — pre-impl check note added; ProofForwardingFact DU variant count corrected from 4 to 5 — adding UnreachableRowFact as the 6th; producer/consumer-inversion concern on ProofForwardingFact explicitly acknowledged in Inventory § F-LANG-SPEC-12 routing.)
...
- `ProofEngine.Intervals.cs`:
  - **Pre-impl check (per reviewer remediation 2026-05-26)**: `NumericInterval.IsEmpty` ALREADY exists at `src/Precept/Language/NumericInterval.cs:11` as `public bool IsEmpty => Max < Min` — verify the existing semantics match before adding. Add `public static NumericInterval Empty { get; } = new(0, -1)` if not present. Existing `Unbounded` sentinel (line 20) coexists.
...
- F-LANG-SPEC-12 routing:
  - New `ProofForwardingFact` variant in `StateGraph.cs:91`: `public sealed record UnreachableRowFact(string FromState, string EventName, ImmutableArray<int> UnreachableRowIndices) : ProofForwardingFact`. (Corrected via reviewer remediation 2026-05-26: the DU already has FIVE sealed variants — `ReachabilityFact`, `DominancePathFact`, `EventCoverageFact`, `TerminalCompletenessFact`, `DeadEndStateFact`; adding `UnreachableRowFact` as the sixth follows the established pattern.)
  - **Producer/consumer architecture acknowledgement (per reviewer remediation 2026-05-26)**: `ProofForwardingFact` was originally a graph-analyzer→proof-engine channel (analyzer produces facts; proof engine consumes them in `IncorporateForwardingFacts`). This design extends the directionality so the proof engine ALSO produces a `ProofForwardingFact` (the new `UnreachableRowFact`), creating a producer-and-consumer overlap on the same DU. The alternative was to introduce a separate `ProofEngineFact` DU. Decision: extend the existing DU rather than fork, because (a) the consumption path (`IncorporateForwardingFacts`) already handles `ProofForwardingFact` subtypes uniformly; forking would require parallel consumption infrastructure; (b) the satisfiability scan IS a graph-routing-relevant verdict (a dead row affects state reachability), so the fact-shape conceptually belongs with the analyzer's facts even though its producer is the proof engine. Accept the directionality inversion; document it in `docs/compiler/proof-engine.md` § Forwarding facts when the doc-sync ships.
...
None. PRE codes 153, 154, 155 verified free against `DiagnosticCode.cs` — current top is actually PRE0156 (`AlwaysFalsePeriodComparison`, W-E); 153/154/155 fill the gap between PRE0152 (W-H) and PRE0156 (W-E). `ProofForwardingFact` DU extension verified open at `StateGraph.cs:91` — already FIVE sealed variants (`ReachabilityFact`, `DominancePathFact`, `EventCoverageFact`, `TerminalCompletenessFact`, `DeadEndStateFact`); adding `UnreachableRowFact` as the sixth follows the pattern. Reviewer-flagged producer/consumer-inversion concern addressed in Inventory § F-LANG-SPEC-12 routing. All decisions carry stakes classifications, four legs, and source citations with verbatim excerpts. Falsifiers section present (5 entries). Doc-update enumeration present (9 entries). No `Stakes: irreversible` decisions; no cooling-off required.
