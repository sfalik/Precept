...
> **Promoted to:** 2026-06-04 — `docs/language/precept-language-spec.md` (§2.3/§2.4 modifier↔type table + discharge note + flag-modifier row + §0.6 count narrative), `docs/language/collection-types.md` (Constraint Catalog, per-kind discharge lines, element-value-modifiers section, `sortedset`-rejection rationale), `docs/compiler/proof-engine.md` (`mincount`-discharge strategy), `docs/compiler/type-checker.md` (`BuildElementValueBounds` generic satisfaction-shape binding), `docs/compiler/diagnostic-system.md` (PRECEPT0031 analyzer roster). Working cross-link: `docs/Working/collection-inner-type-value-modifiers-2026-06-02.md` (Supporting Decision 3 / Falsifier 5 retargeted). **Canon synced in ship commits `c3209ad2` (catalog retarget + generic element binding + `mincount` discharge) and `eeb9a9c6` (PRECEPT0031 no-overlap analyzer — type `Precept0029ModifierAxisDisjoint`, diagnostic id `PRECEPT0031`).** Archival-only: no rationale required lifting at promotion time.
