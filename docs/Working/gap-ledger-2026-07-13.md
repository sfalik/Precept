...
>
> **Disposition:** OF4 is closed as **already-answered by locked D14**. Follow-up is a doc-fix — reconcile `collection-types.md:533` to D14 (remove the `period in 'days'` ordering grant / mark equality-only) — proposed to the owner, not a fresh ruling. *(The re-check also claimed a `set of money` unqualified `.max` "false-clean" — that is overstated: verified at HEAD 2026-07-24, `Prices.max >= Prices.min` on an unqualified `set of money` **errors** via `UnprovedQualifierCompatibility` (currency unresolved), so no bad program compiles. `collection-types.md:530` wants the `.max` accessor itself rejected as a type error; HEAD instead offers the accessor and catches misuse downstream — a diagnostic-placement divergence, sound, not a soundness bug.)*
