...
> ## ⚠️ Correction (2026-06-01) — this design's Principle-11 claims were overstated
>
> This doc claimed to "close" the declared-bound Principle-11 gaps. That claim was wrong, and it shipped because the doc **rewrote Principle 11**:
> - The Philosophy-Alignment P11 row qualified static-completeness as holding *"on statically-decidable values"* — **a qualifier the spec does not contain** (spec:111 / Principle 11 is unqualified: *"if a precept compiles without diagnostics, it does not fault at runtime"*). That qualifier let D4 claim "P11 strengthened" while a real hole stayed open.
> - The doc treated the unbounded-result skip as *"the same posture numeric set-actions already take … by-design"* (citing proof-engine.md:1680) — i.e. it cited **the exact behavior now filed as BUG-017** as legitimizing precedent. That behavior is a Principle-11 **violation**, not an accepted boundary.
>
> **Honest status of what 3b (`ecce5d51`) delivered vs claimed:**
> - **Delivered (genuine, behavior-preserving):** the `OutOfRange` relocation to a proof obligation, killing the arg double-emit + field/arg split; the `NumericOverflow`/`OutOfRange` Type→Proof relabels; string-length default closure (D8); the qualifier-residual relocation for walked sites (D5).
> - **NOT delivered (claimed but open):** D4's "close the computed-field-result-vs-bounds Principle-11 gap." D4 closed only the case where the result interval is **bounded**; when the result is **unbounded** the check is silently skipped — **BUG-017**, an open soundness hole. The "deferred / conservative boundary / not a silent hole" framing below is the rewrite and is corrected inline.
>
> Do not treat the P11-completeness claims in this doc as true. The corrected statements are inline below; the open hole is BUG-017.

## Goal
When done, every "a declared value/result violates a declared bound" condition — numeric, string-length, or collection-count, **including `notempty`** (the length/count ≥ 1 lower-bound case) — is owned by exactly one stage and emits exactly one code, with no double-emit, no field-vs-arg split, ~~no silent gap on a statically-decidable value~~, and **no dead `[StaticallyPreventable]` code**; demonstrated by a per-family per-cell single-emission test matrix over {field default, arg default, computed field, set-action} × {literal, typed-constant, resolvable interpolated/list-literal, non-constant}. **(Corrected: the "no silent gap *on a statically-decidable value*" clause carried the same Principle-11 rewrite — it scoped the unbounded computed/assigned-result case out of the goal. That case is a real silent gap, BUG-017; this goal does not cover it.)**
...
- **Out of scope** (enumerated cells that need machinery beyond this slice — the first is an open soundness hole, the rest are genuinely-not-holes):
  - **Non-literal *computed* string-length / collection-count narrowing** — a `field Name as string maxlength 10 <- First + Last`, or a computed collection result, cannot be bounded by the literal-only provers. **CORRECTED: this is an open soundness hole, not an "accepted boundary."** The original text here claimed it was "not a silent hole … the same literal-only conservative boundary the numeric set-action path already accepts (proof-engine.md:1680)" — that was the rewrite (see the Correction at top): a result that can violate its declared bound and compiles clean *is* a silent hole and a Principle-11 violation. The numeric form is **BUG-017**; the length/count form is the same shape. Closing it needs length/count interval (abstract) domains (a Slice-5-class build); it is genuinely deferred, but as an **open soundness gap**, not an accepted limitation.
...
- **Deferred to future**: the set-action / computed-field `IntervalTransfer`-unbounded **soundness holes** (**BUG-017** — a result that can exceed its bound compiles clean; *not* "conservative," an open Principle-11 violation); length/count interval domains (above); any later merge of the family codes (explicitly rejected — D1).
...
| 11. Static completeness | Y — **partially; see Correction at top** | Genuine P11 fixes: reviving the dead `CountBoundViolation` (D9) and the literal-default closures. **NOT fixed:** the computed/assigned-result-vs-bound gap is closed only when the result interval is **bounded**; an **unbounded** result compiles clean (**BUG-017**) | Unqualified P11 (spec:111) is **not** met — an unbounded computed/assigned result can fault at runtime with no diagnostic | **Open soundness hole (BUG-017)** — not an accepted boundary. The original row's *"on statically-decidable values"* qualifier and *"same boundary numeric set-action already accepts"* were a rewrite of the principle |
...
- **P11 (static completeness):** *partially strengthened — and one claim here was a rewrite (see Correction at top).* Genuine: reviving the dead `CountBoundViolation` and closing the literal-default gaps removed real spec:111 violations. **But NOT "strengthened across the whole family":** when a computed/assigned result's interval is **unbounded**, the bound check is silently skipped and the result can fault at runtime with no diagnostic — **BUG-017**, an open Principle-11 violation. The original text here qualified P11 to *"statically-decidable values"* and called the hole a *"documented literal-only conservative boundary … the same posture numeric set-actions already take"* — that qualifier is not in spec:111, and the cited "posture" is the bug itself. P11 (unqualified) is **not** satisfied by this slice; the gap is BUG-017.
...
> **CORRECTION (2026-06-01): D4 did not deliver this title.** It closed the case where the result interval is **bounded** (verified: a computed field whose bounded result exceeds its `max` now emits `NumericOverflow`). It did **not** close the **unbounded** case — `IntervalContainment` silently skips an unbounded result, so a computed field that can exceed its bound still compiles clean. That is **BUG-017**, an open Principle-11 violation — and it is precisely the *"narrow silently"* alternative this decision's own "Alternatives considered" leg rejected as *"the P11-forbidden status quo."* D4 inherited that status quo for unbounded results while claiming to have closed it. Treat D4 as **partial**: bounded-result coverage added; the P11 gap is still open (BUG-017).
...
> **Execution note (not a design change — see Open Decision in the plan).** 3b's build relocated **default + computed-expr** sites; min/max **bound expressions** were left type-emitted because the proof engine does not currently walk bound expressions (stamping there would drop the diagnostic). Whether bounds should be walked, split to a distinct code, or accepted as an analyzer carve-out is an **unresolved design decision surfaced during execution** — it is NOT settled here. Tracked in the plan's Open Decisions; to be resolved before Slice 3c (which depends on the PRE0141 ownership story). Also observed: PRE0141 proof-ownership is not visible via `Diagnostic.Stage` (its meta stage is always Proof) — obligation-presence is the observable signal.
...
- **Strongest counter-evidence**: the literal-only prover can't handle interpolated/computed string defaults, so the "gap closure" is partial. *Response (corrected)*: literal/typed-constant defaults are the common case and the concrete bug, and those are closed. The non-literal *computed* cell is genuinely deferred — but it is the **same open soundness hole as BUG-017** (a computed result that can violate its length bound, compiling clean), **not** a "literal-only-by-design posture" to lean on. (The original wording cited proof-engine.md:1680 as legitimizing precedent; that precedent is the bug.)
