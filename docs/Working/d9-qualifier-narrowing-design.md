...
**Soundness preservation.** Principle 1/11: the discharge admits a program iff the guard *provably* establishes the required value on every reachable branch — strictly narrower than "accepted." Failure modes that would break soundness (value-mismatch discharge, OR-arm collapse, negative-equality false-positive, cross-branch leak, declared-qualifier override, field-to-field false value, datetime over-credit, Any-discharge) are each closed by a stated rule (D3) and a required guard test (analysis § failure-mode catalog). Principle 7: cross-partition and unprovable cases are rejected at compile time.
...
- Tooling (completions, hover, semantic tokens): completions after `X.dimension == '` offer the partition-correct set; after `X.currency == '` offer ISO 4217. Hover: None new.
- MCP (vocabulary, DTOs, tool output): None.

**Breaking changes.** None to public contracts. Previously-failing programs (PRE0141/0113/0053 on valid guarded operations) now compile (relaxation). `quantity.dimension == 'date'` stays rejected. No catalog renames; the new `QualifierAxis` consumers are additive.
...
- **Precedent**: the numeric narrowing path (`GuardConstraint` + `NumericConstraintSubsumes`, `Strategies.cs:870-898`) is the structural template; this adds a sibling, not a mutation.
...
### Decision 3: Soundness rules are hard gates — value-exact discharge, all-branches-OR, intersection-with-declared, literal-RHS-only, negation-narrows-nothing
**Stakes**: high
- **Rationale**: Each rule closes a concrete unsoundness (analysis § failure-mode catalog). Value-exact discharge (compare narrowed value to required value) closes the #1 trap; all-branches-OR closes OR-arm collapse; intersection-with-declared closes contradiction-override; literal-RHS-only and negation-narrows-nothing close field-to-field and `!=` false positives. Without these, the feature violates Principle 1.
...
- **Precedent**: the numeric path's `ValueSatisfiesRequirement` value-exactness (`Strategies.cs:881`), all-branches loop (`:660-677`), and `// OR: do NOT decompose` carve-out (`:766`).
...
- **Strongest counter-evidence**: authors will hit "my OR-guard didn't narrow" friction. Looked at the numeric path — it has the same friction and accepts it. Response: a teachable diagnostic ("guard narrows to a set {USD,EUR}; the field requires exactly USD") mitigates; soundness is not negotiable.
...
- If any of the 11 failure-mode tests (analysis § catalog) cannot be made to fail-before / pass-after, the soundness model (D3) is incompletely implemented.
...
- Every failure-mode in the analysis catalog has a fail-before/pass-after test.
...
## Open questions
None blocking. (Two design choices are resolved inline and flagged for the reviewer rather than left open: the fact-vehicle is D1; the basis representation is D5. The unitofmeasure `.dimension` scope is settled in D7. Interpolation-slot narrowing from open fields is explicitly out of scope, not open.)
