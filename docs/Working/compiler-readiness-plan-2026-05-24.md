...
| 2 | **Decide:** how should `price × quantity` cancel across same-dimension / different units? (`price in 'USD/kg' × quantity in 'g'`). The impl matches at **dimension** granularity and tracks no conversion factor — Slice 1 made this cancel silently (scale-factor hole, now live). Spec is in tension (price exact-unit `:1866` / "conversion is explicit" `:168` vs quantity-arithmetic auto-convert D8 `:445–452` vs count-unit reject PRE0137 `:397`). Genuine semantics decision, not a bug. **Evidence + 3-option decision space:** [`price-cross-unit-cancellation-2026-05-31.md`](price-cross-unit-cancellation-2026-05-31.md). Decide via `/lifecycle-2-design`. | Next — decision parked; hole LIVE |
