...
| 8 | **Diagnostic-emission architecture** (Slice 1 inventory ✅ / Slice 2 code-mediation standardization ✅ `59fe2666` / Slice 3 ownership architecture — 3a taxonomy+consolidation ✅ `f7e5dee6`, 3b declared-bound ownership **design-locked** (`phase8-value-level-obligation-ownership-2026-06-01.md` — expanded to the whole value-vs-bound family) + 3b-count, 3c analyzer / Slice 4 counterexample witnesses / Slice 5 `maxplaces` precision proof — from the 2026-05-29 type↔proof-contract survey) | 5 slices | 2 | M→L | **Active — taken up 2026-05-31** |
...
*(Split from Diagnostic completeness 2026-05-31 — the emission-architecture work (Slices 1–4 below) is a distinct job from the catalog-completeness sweep (now Phase 9) and is being taken up first, ahead of both Phase 9 and the close of Phase 7.)*
...
**Grounding**: a Whiley deep-dive — the closest structural analog (a language designed around a separate verification stage that self-derives, not VC/IVL) — would best inform Slices 3-4 if deeper grounding is wanted; not a build item. The companion architecture-direction conclusion — *stay catalog/stamp; no VC-gen/IVL; no incremental* — is recorded in `research/architecture/README.md` open-questions #1/#2.

## Slices (execution sequence)
...
- [`phase8-slice1-emission-inventory-2026-05-31.md`](phase8-slice1-emission-inventory-2026-05-31.md) — Phase 8 Slice 1: diagnostic-emission inventory & classification (grounds Slices 2–3).
...
- [`phase8-value-level-obligation-ownership-2026-06-01.md`](phase8-value-level-obligation-ownership-2026-06-01.md) — Slice 3b's dedicated `/lifecycle-2-design` (Locked, 3 review rounds): the complete declared-bound family (numeric/length/count/notempty) single-owned; D1 keep-both-codes partition, D2 OutOfRange via stamped Numeric, D4 computed-field gap, D8 length defaults, D9 revive dead `CountBoundViolation`, D10 no-collision. Expands Slice 3 D3, resolves D5.
