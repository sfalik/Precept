...
**Status**: Investigation — superseded by the **Locked design** [`price-cross-unit-cancellation-design-2026-05-31.md`](price-cross-unit-cancellation-design-2026-05-31.md) (Locked 2026-05-31: auto-convert within dimension; Decision 2 = allow-all-with-surfacing, no exactness gate). This doc remains the evidence base / decision trail. **The hole is currently LIVE** as of the Slice 1 fix: Slice 1 fixed same-unit `price × quantity` cancellation and, by owner decision (option C), left the cross-unit case open — it currently cancels silently, dropping the conversion factor; the auto-convert decision closes it. A skipped test (`PriceTimesQuantityTests.CrossUnit_SameDimension_MustNotSilentlyCancel`) marks the gap in the suite.
...
- **Log units as amounts — position-rule-fine; cross-scale allowed and surfaced approximate.** A dB *gain* is an amount (not an absolute position), so the position rule does not exclude it, and priced **same-unit** (`$5/dB × 10 dB`, factor 1) it cancels exactly. dB *cross-scale* conversion (dB↔Np) has an **irrational** factor (8.686 = 20/ln 10), so per the owner's allow-all ruling it is **allowed and surfaced as approximate** (not rejected), like angle→radian. *(An earlier draft here said dB "converts to nepers cleanly and prices consistently" via ÷8.686, then a subsequent draft said an exactness gate rejected it; both are superseded — the conversion is allowed, and its approximate status is made visible in inspection.)*
- **What the position rule excludes: nothing for `quantity` amounts.** kg, °C-of-change, and dB-gain are all amounts. Cross-scale admissibility is then governed by the allow-all-with-surfacing rule: °C↔°F (×1.8, exact rational) admitted and surfaced exact; dB↔Np (irrational) admitted and surfaced approximate. A cross-unit cancellation is admitted whenever the operands are commensurable amounts (ratio-scale, same dimension); exactness is surfaced, not gated.
