...
A constraint is **never routed to one role or the other** — it always carries both. Crucially, the two roles often act on **different targets**: a single rule can *prove* a derived value (premise role, at compile time) while *governing* its own external fields (governance role, at the sweep). Examples A and H in §6 demonstrate this in two different domains.
...
Eleven examples, each in real `.precept` syntax, each naming the provenance of every field explicitly and walking every obligation to its enforcement point. They are deliberately drawn from different domains — money, decimals, instants, quantities with units, strings, and collections — and different constraint kinds — numeric bounds, relational rules, guarded temporal rules, state and event ensures, collection cardinality, and string length — so that the classification is seen to run on provenance alone, never on the type of the value or the spelling of the constraint. No single example is *the* pattern; the procedure of §5 is, and it lands identically on every shape below.

### A. One rule, two roles — a derived value proven by a constraint that is itself governed
...
### G. Temporal governance — guarded ordering rules and a state ensure, no number in sight

```precept
precept RegistrationWindow

field RegistrationOpens as instant optional
field RegistrationCloses as instant optional
field SubmittedAt as instant optional

rule RegistrationCloses > RegistrationOpens when RegistrationOpens is set and RegistrationCloses is set because "Registration close {RegistrationCloses} must be after open {RegistrationOpens}"

state Draft initial
state Submitted terminal

in Draft modify RegistrationOpens, RegistrationCloses editable

in Submitted ensure SubmittedAt is set because "A submitted registration must carry a submission timestamp"

event Submit(At as instant)

from Draft on Submit
    -> set SubmittedAt = Submit.At
    -> transition Submitted
```

**Provenance.** `RegistrationOpens` and `RegistrationCloses` are **independently-set** — editable in `Draft`, each supplied by the registrar on its own schedule, neither computed from the other. `SubmittedAt` receives an externally-supplied event argument by direct assignment. Nothing here is derived by arithmetic; nothing here is a number. (This is the shape of [`../samples/academic-course-registration.precept`](../samples/academic-course-registration.precept), condensed.)

**The ordering rule is Example C in the temporal domain.** `RegistrationCloses > RegistrationOpens` is a relational invariant over two external fields — the identical classification as `Balance >= MinBalance`, reached by the identical procedure, with instants in place of money. Ingress cannot see it (each edited instant is impeccable by itself); the prover has no subject (no value is computed); the **post-mutation sweep** enforces it on every operation. An edit dragging `RegistrationCloses` before the currently-set `RegistrationOpens` builds a working copy the rule condemns; the copy is discarded whole and the caller receives `ConstraintsFailed`. The router never looked at the type — provenance classified it before "instant" was even relevant.

**The `when … is set` guard does not weaken anything.** Both fields are `optional`; the guard scopes the rule to configurations where both are populated. The sweep evaluates the *whole* guarded rule against the completed working copy: a copy with only one instant set satisfies it vacuously, a copy with both set is held to the ordering — in both directions, on every operation. A guarded rule is a precisely-scoped contract, not a partially-enforced one.

**The state ensure is governance keyed to a region of the lifecycle.** `in Submitted ensure SubmittedAt is set` is a declared constraint over an externally-populated field, scoped to a state. It is enforced exactly like a rule — at the sweep, against the completed working copy — whenever that copy is (entering or remaining) in `Submitted`. Here the `Submit` row sets `SubmittedAt` before transitioning, so the ensure holds by construction on that path; had the author written a path into `Submitted` that left `SubmittedAt` unset, the sweep would refuse it with `ConstraintsFailed`. Presence-in-a-state is a constraint kind with no numeric content at all, and it classifies without friction: external field, declared constraint, governed.

### H. One ensure, two roles — an event-scoped relational constraint in the quantity domain
...
event Receive(Qty as quantity in 'kg' positive)
...
on Receive
    -> set QuantityOnHand = QuantityOnHand + Receive.Qty

on RecordShrinkage
    -> set QuantityOnHand = QuantityOnHand - RecordShrinkage.Qty
```

**Provenance.** `RecordShrinkage.Qty` is **externally supplied** — an event argument. `QuantityOnHand` accumulates externally-supplied amounts; its running value originates outside the definition. The RHS `QuantityOnHand - RecordShrinkage.Qty` is a **derived** value flowing into a field bounded `nonnegative`. (This is the shape of [`../samples/inventory-item.precept`](../samples/inventory-item.precept), condensed to a single unit.)

**Three obligations, three enforcement points.**

- **Ingress** governs the argument's own contract: a `RecordShrinkage` carrying a zero or negative `Qty` is refused at the door (`InvalidArgs`) — `positive` is checked per-value, before anything runs.
- **The event ensure** is a relational constraint between an argument and the current state — a *combination* that per-value ingress structurally cannot see, exactly as a cross-field rule is. Its governance role fires in the post-fire constraint pass: a `RecordShrinkage` attempting to write off more than is on hand fails the ensure, the working copy is discarded whole, and the caller receives `ConstraintsFailed` carrying the `because` text ([`runtime/result-types.md`](runtime/result-types.md) — event ensures surface through the same outcome as rules and state ensures). Being event-scoped narrows *when* it is evaluated, not *how absolutely* it binds.
- **The subtraction** is a prove-or-reject obligation: prove `QuantityOnHand - RecordShrinkage.Qty` lands in `[0 kg, +inf)`. From the field bound and the argument bound alone, the difference is unbounded below — undischargeable. The ensure's **premise role** supplies the missing relational fact on exactly the path where the subtraction runs: under `RecordShrinkage.Qty <= QuantityOnHand`, the difference is provably non-negative, and the obligation **discharges at compile time**.

**This is Example A's dual-role structure in a different domain and a different carrier.** There, a definition-wide `rule` proved a settlement balance; here, an event-scoped `ensure` proves an inventory decrement — quantities with units instead of money, an ensure instead of a rule, and the same clean division: the derived write is proven, the external relationship is governed, and one declared constraint does both. The pattern is the model's, not the bank account's.

### I. Collection cardinality — access preconditions proven from guards, cardinality governed at the door
...
**Provenance.** The queue's *contents* are externally supplied (enqueued event arguments); the queue's *count* at any moment is a fact about the entity's accumulated configuration. The fault-prone site is the **operation**: `dequeue` on a possibly-empty queue is an empty-collection access — one of the enforced fault classes of §3, as structural as division by zero. (This is the shape of [`../samples/it-helpdesk-ticket.precept`](../samples/it-helpdesk-ticket.precept), condensed.)

**The access is prove-or-reject, and the guard is its carrier.** `dequeue AgentQueue` stamps a compile-time obligation: prove the queue is non-empty wherever this row runs. The `when AgentQueue.count > 0` guard establishes exactly that fact on the path, and the obligation **discharges from the guard**. Strip the guard and the obligation is undischargeable — the definition is **rejected**, naming the guard that would fix it. Never accepted-and-caught-at-runtime: an empty-dequeue "refusal" would be addressed to no one (the caller supplied no queue), which is the banned arm's tell (§7).

**The unguarded sibling row is the authored disposition for the uncovered case.** A `Fire("Assign")` on an empty queue matches the second row and returns the author's own refusal (`EventOutcome.Rejected`, in the author's words) — the same completeness move as Fix 3 in Example K: every reachable case has a disposition the author wrote down.

**Cardinality bounds on external collection values are governed at ingress.** When a whole collection is *supplied from outside* — a construction input such as `CatalogItems as set of string notempty mincount 1` (the shape of [`../samples/shopping-cart.precept`](../samples/shopping-cart.precept)) — its declared cardinality is a per-value contract like any other: a caller constructing with an empty set is refused at the door, before any working copy exists. Same constraint kind, two provenances, two dispositions — the count *consumed by an operation inside the definition* is proof material; the count *of a value crossing the boundary* is ingress material. The procedure of §5 lands each without ambiguity.

### J. String-length containment — a decidable bound that is not a number line
...
**Provenance.** `Approve.Advisor` is **externally supplied**; the assignment flows it into `AdvisorName`. As in Example B, the write's containment obligation is discharged — or not — from the constraint the value *carries*.

**The rejection.** `AdvisorName` declares `maxlength 50` — a decidable bound, as decidable as `max 100`; length is simply the measure. The argument carries `maxlength 100`: the carried range (length ≤ 100) is **not** contained in the target bound (length ≤ 50), a 60-character advisor name is a live counterexample, and the obligation is undischargeable. The definition is **rejected at compile time**, naming the mismatch. The honest fixes are the usual ones — tighten the argument to `maxlength 50`, or widen the field — and each puts the truth in the declaration where both the prover and every reader can see it.

**The banned arm has a string-domain costume too.** "Accept the definition and truncate at runtime" — or "accept it and refuse the operation when the name comes out too long" — is governing a value the definition itself flowed into the field, with no honest addressee for the refusal and no visible declaration of the loss. Precept rejects the definition instead. Silent truncation is precisely the kind of quiet approximation the honesty commitment forbids ([`philosophy.md`](philosophy.md)): the contract says 50, the input contract admits 100, and the mismatch is the author's to resolve in the source, not the runtime's to absorb.

**And the governed half, for contrast:** with the fix applied (`Advisor as string notempty maxlength 50`), the containment discharges trivially from the carrier — and **ingress** is what makes the carrier true of every actual value: a caller supplying an empty name or a 51-character name is refused at the door with `InvalidArgs`. Proven write, governed source — the same seam as Examples B and E, measured in characters instead of currency.

### K. The banned third arm — an unprovable computed value, and the three honest fixes
...
The rule's **premise role** discharges the containment obligation at compile time (Balance is proven); its **governance role** enforces the relationship on the external totals at the sweep. This is Example A — the resolution that most enriches the contract: the domain truth ("you cannot withdraw more than you deposited") is now declared, proven against, and governed.
