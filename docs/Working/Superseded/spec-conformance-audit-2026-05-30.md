...
- **E4 — `price × quantity` count-mismatch cancels silently (soundness hole).** `price in 'USD/each' × quantity in 'box'` cancels with no diagnostic. `PRE0137 CrossCountingUnitOperation` is gated on **both** operands being `TypeKind.Quantity` (`TypeChecker.Expressions.cs:1351`, `…Callables.cs:895`), so the price-cancellation path skips it — count units with no universal factor cancel on the shared `count` dimension name. Same class as the cross-unit hole. **CONFIRMED** (probed; characterization test `CrossCountingUnit_PricePath_CurrentlyCancelsSilently_KnownGap` pins current behavior). *Route the price-cancellation path to PRE0137 — own slice (alongside Slice 6 log, or its own).*
