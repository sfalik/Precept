...
The Tokens catalog (`Tokens.All`) is the exhaustive inventory of every token the lexer can produce. Each entry carries `TokenKind`, surface text (for keywords/operators/punctuation), categories, description, `TextMateScope`, `SemanticTokenType`, and `ValidAfter`. The lexer derives its keyword lookup table from this catalog at static initialization. Operator and punctuation recognition uses hardcoded scan methods (`TryScanOperator`, `TryScanPunctuation`) whose entries match the catalog — the catalog remains the single source of truth for what operators and punctuation exist.

### No Parallel Copies

If the lexer switches on token kinds to apply per-token behavior, that behavior belongs in catalog metadata — not in lexer code. `TryScanOperator` and `TryScanPunctuation` use hardcoded scan logic whose entries correspond one-to-one with catalog metadata. Adding a new operator or punctuation token requires both a catalog entry and a scan-method update.
...
`TryScanOperator` uses hardcoded if/else chains for multi-character operators (`->`, `!~`, `~=`, `==`, `!=`, `>=`, `<=`) and a switch expression for single-character operators (`=`, `>`, `<`, `+`, `-`, `*`, `/`, `%`, `~`). Multi-character operators are checked first to ensure maximal munch (e.g., `>=` is not mis-scanned as `>` then `=`). The Tokens catalog carries the surface text and categories for every operator; the scan method entries correspond one-to-one with catalog metadata.

### Punctuation

`TryScanPunctuation` uses a hardcoded switch expression for `.`, `,`, `(`, `)`, `[`, `]`. Delimiter characters (`{`, `}`, `"`, `'`) are not in this switch — they have dedicated mode-transition handling in `ScanToken`. The Tokens catalog carries the metadata for every punctuation token; the switch entries correspond one-to-one with catalog metadata.
...
`TryScanOperator` checks multi-character operators first (if/else chain), then single-character operators (switch expression). This ordering ensures maximal munch — `>=` is recognized as one token, not `>` followed by `=`. Arrow (`->`) is included because it is a multi-character structural token recognized by the same scanner. The Tokens catalog classifies operators with `TokenCategory.Operator`; arrow is `TokenCategory.Structural`.

### Punctuation

`TryScanPunctuation` uses a switch expression mapping six single characters (`.`, `,`, `(`, `)`, `[`, `]`) to their `TokenKind` values. Delimiter characters (`{`, `}`, `"`, `'`) are excluded — they trigger mode transitions in `ScanToken` and are never emitted as punctuation tokens. The Tokens catalog classifies these with `TokenCategory.Punctuation`.
