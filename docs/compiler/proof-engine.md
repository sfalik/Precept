...
**No public-surface change.** Both fact records are private to the proof engine and never serialized; the relational mechanism adds no `ProofRequirement`, `ProofRequirementKind`, `ProofStrategy`, or DTO. Relational discharge is attributed via the existing `ProofStrategy.FlowNarrowing`, preserving the named-rule witness.
