...
- [2. Overview](#2-overview)
  ...
  - [SlotValue Subtypes](#slotvalue-subtypes)
  - [Blocking Dependency: Expression Trees (RESOLVED)](#blocking-dependency-expression-trees-resolved)
...
- [5. Inputs and Outputs](#5-inputs-and-outputs)
  - [Input](#input)
  - [Output](#output)
- [6. Architecture: Catalog-Driven 2-Pass Design](#6-architecture-catalog-driven-2-pass-design)
  - [The Dispatch Model](#the-dispatch-model)
  - [Symbol Table (from NameBinder)](#symbol-table-from-namebinder)
  - [Checking (Expression Resolution + Normalization + Structural Validation)](#checking-expression-resolution--normalization--structural-validation)
- [7. Component Mechanics](#7-component-mechanics)
  - [7.1 SemanticIndex Shape](#71-semanticindex-shape)
  - [Collection Type Decision](#collection-type-decision)
  - [Record Types](#record-types)
  - [7.2 CheckContext: Mutable Working State](#72-checkcontext-mutable-working-state)
  - [7.3 Expression Resolution (UNBLOCKED)](#73-expression-resolution-unblocked)
  - [Catalog Lookup Strategy](#catalog-lookup-strategy)
  - [Qualifier Disambiguation Logic](#qualifier-disambiguation-logic)
  - [Type Widening Integration](#type-widening-integration)
  - [Binary Operation Widening Fallback](#binary-operation-widening-fallback)
  - [Numeric Literal Context Resolution](#numeric-literal-context-resolution)
  - [Function Overload Resolution](#function-overload-resolution)
  - [Accessor Return-Type Resolution](#accessor-return-type-resolution)
  - [Identifier Resolution Priority](#identifier-resolution-priority)
  - [Stub Strategy for Unimplemented Arms](#stub-strategy-for-unimplemented-arms)
- [Catalog Integration (part of §7)](#catalog-integration-part-of-7)
...
- [12. Innovation](#12-innovation)
  - [Catalog-Driven Expression Resolution](#catalog-driven-expression-resolution)
  - [Generic Construct Dispatch](#generic-construct-dispatch)
...
**Element value bounds.** The base `TypedElementType` carries an optional `DeclaredValueBounds? ValueBounds` companion (a DU-subtype extension, not a nullable-flattened bound representation) holding the value modifiers declared on the inner type — `queue of string maxlength 200` resolves to a `TypedScalarElement(String) with { ValueBounds = DeclaredValueBounds(DeclaredMaxLength: 200) }`, and `set of integer min 0 max 100` to `DeclaredValueBounds(DeclaredMin: 0, DeclaredMax: 100)`. `DeclaredValueBounds` reuses `TypedField`'s bound vocabulary — length (`DeclaredMinLength`/`DeclaredMaxLength`), numeric range (`DeclaredMin`/`DeclaredMax` + their `NormalizedDeclaredMin`/`NormalizedDeclaredMax` UCUM-base variants for qualified elements), and the numeric sign-flag kinds (`nonnegative`/`positive`/`nonzero`, carried as a `NumericFlags` list) — so the proof engine's length- and interval-containment machinery reads field bounds and element bounds through one shape. `BuildTypedElementType` unwraps the parser's `ElementValueModifiedTypeReference`, builds the base element type from the underlying inner reference, then attaches the validated bounds. Validation runs through `BuildElementValueBounds`, which extracts the element's declared qualifiers (for `money`/`quantity` bound normalization, via the same `TryGetComparableModifierValue` path a qualified field uses) and calls the **same** `ValidateValueModifiers` path as a field/arg declaration with the element's `TypeKind` as subject — so `set of integer maxlength 5` and `set of string min 0` each emit `InvalidModifierForType` (PRE0033) per element exactly as the same modifier on a scalar field would, with no parallel validator and no second compatibility table. The bound projection is **catalog-generic**: `BuildElementValueBounds` loops over each modifier's `ProofSatisfactions` and dispatches on the satisfaction DU shape `(Projection, Comparison, Bound)` — not on `modifier.Kind` — mirroring `FlagLowerBoundFromMeta`. `Accessor("length") + DeclarationValue` fills the length slot, `SelfValue + DeclarationValue` the numeric slot (UCUM `Declared`/`Normalized` split preserved), `Accessor("length"|"count") > Constant(0)` sets `NotEmpty`, and `SelfValue + Constant` appends a sign flag; a modifier with no matching Numeric satisfaction (`maxplaces`/`optional`/`default`/`ordered`) contributes nothing. The routed modifier set is catalog-derived (`Modifiers.ElementPositionValueTokens` — the scalar value modifiers that apply to no collection type). `notempty` is now string-only, so it routes to the element: `set of string notempty` binds each element non-empty (`NotEmpty = true`, folded to `minlength 1`), identical to `set of string minlength 1`; collection cardinality is the separate `mincount`. `notempty` on a non-string element (`set of integer notempty`) emits `InvalidModifierForType`. A quantifier binding over a bounded-element collection carries the element band on its resolved `TypedFieldRef.ElementBounds`, so the proof engine seeds the binding's value interval from the inner-type bound.
...
A compound `price in 'currency/unit'` qualifier (`MapPriceInQualifier`) validates the denominator against three registries in order: **count** units (`each`/`box`/… → dimension `count`); **NodaTime temporal** units (`hours`/`minutes`/`days`/… — the vocabulary § D15 mandates for time-unit denominators — with the dimension derived from `TemporalUnits.IsCalendarBased`: clock units → `time`, calendar units → `date`); then **UCUM** physical units (`kg`/`L`/… → the UCUM-derived dimension). The temporal-dimension spellings (`time`/`date`) match what `.dimension`/`TemporalDimension` resolve to, so a temporal-denominator price (`price in 'USD/hours'`) cancels a `period` declared with a temporal **dimension** (`period of 'time'`) or a `duration` through the existing `QualifierChainProofRequirement`. A period declared only by **basis** (`period in 'hours'`) does **not** yet cancel — the chain resolves the period's `TemporalDimension`, which a declared `TemporalUnit` basis does not surface on that path (a known gap against § D15, which says `period in 'hours'` should cancel `price in 'USD/hours'`; `period of 'time'` is the working form meanwhile). Cancellation discharge is currently at **dimension** granularity and does not yet *reject* denominator/legal-basis mismatches for declared operands (e.g. `price in 'USD/hours' × period in 'days'`, or `price in 'USD/kg' × quantity in 'L'`) — that tightening is a separate enforcement concern, not part of denominator parsing.

`period` carries the `TemporalUnit` axis in the assignment-qualifier check, so the D14 composite-basis constraint is enforced on assignment: a value assigned to a `period in '<basis>'` field is accepted iff its non-zero component set is a **subset** of the declared basis (`business-domain-types.md` § D14), checked by `ResolvedQualifierSatisfiesTarget`'s `TemporalUnit` arm (`source.Components ⊆ target.Components`) — subset, not equality, so `{hours} ⊆ {hours, minutes}` is accepted while `{days}` is not. A period **literal** derives its own basis from its non-zero NodaTime components (`DerivePeriodLiteralBasis`, iterating `TemporalUnits.AllEntries` in coarse-to-fine order — consistent with `MapTemporalUnitQualifier`), so a default/literal source is subset-checked like a declared one. An **open** period source (no declared/derived basis) is discharged at the proof stage by a `when X.basis == '…'` guard; that guard's string-literal RHS is canonicalized to the canonical `" + "`-joined basis form here at the type checker (in `CreateResolvedBinaryOp`, gated to a `.basis`-accessor-on-`period` peer) so the proof engine compares already-canonical strings — surfacing the composite-basis diagnostics (PRE0160–0162) on a malformed guard basis.
...
| 20 | Identifier resolution priority | Not addressed | Bare identifier: quantifier bindings > fields. Event args are dotted-only (`EventName.ArgName`, spec §3.5) — a bare reference to an in-scope arg is rejected (`UnqualifiedEventArgReference`, PRE0163); a bare name colliding with a same-named field resolves to the field | A bare name is unambiguous — always a field or binding; event args carry an explicit event qualifier, so no silent field/arg shadowing |
