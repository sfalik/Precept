...
| Constraint `.Message` interpolation holes — **presence** (e.g. `because "value {OptionalField}"`) | `Presence` | **deferred** — owner-fork, still open and deliberately not settled by the 2026-07-23 value-fault ruling. Interpolating a possibly-unset optional is what a failure message is *for*, so enrolling presence here would force authors to guard the field they are reporting on. The recommendation on file resolves it with a language guarantee that message rendering is presence-tolerant — that guarantee exists nowhere in canon and is new language surface under the Pre-Design gate, so it cannot ride along with the value-fault ruling. Re-open trigger: owner ruling on the presence arm. |
