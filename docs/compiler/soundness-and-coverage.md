...
| Constraint `.Message` interpolation holes (e.g. `because "bad {10/0}"`) | value-fault vs. presence obligations | **deferred** — owner-fork: whether message holes enroll, and for which fault classes. Frank's (unratified) recommendation is SPLIT (value-faults enroll; presence exempt via a message-rendering null-tolerance guarantee). Re-open trigger: owner ruling on the message-hole fork. |
