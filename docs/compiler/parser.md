...
*Why Precept, not convention:* The Constructs catalog carries `LeadingToken`, `Slots`, `AllowedIn`, and `UsageExample` per construct. These are the parser's dispatch key, shape skeleton, semantic scoping rule, and diagnostic example text — read from metadata, not duplicated in parser code. *Rejected alternative: hard-coded switch branches with per-construct knowledge inlined into the parser. This scatters domain knowledge across a 2000-line file and forces parallel updates when the language surface changes.*
...
*Why Precept, not convention:* A flat, line-oriented grammar with no block delimiters needs an unambiguous structural signal at the start of each line. Prepositions are the natural English equivalent of a block opener — they scope the line to a state or event context without requiring braces or indentation. *Rejected alternative: indentation-sensitive grammar. This couples layout to semantics, complicates copy-paste, and conflicts with Precept's design principle that the language reads as a policy document, not code.*
...
*Why Precept, not convention:* Precept's grammar is LL(1) with a small number of two-token lookaheads (preposition + verb). No production requires unbounded lookahead. A single-pass design bounds memory to the depth of the expression tree and guarantees linear-time parsing. The token stream is immutable and indexable — `Peek(n)` is O(1), so bounded lookahead adds no allocation cost. *Rejected alternative: parser combinator library (Superpower, Pidgin). Combinators add allocation pressure, obscure the control flow of error recovery, and make catalog-driven dispatch unnatural. The v1 parser used Superpower — the clean-room v2 design moves away from it.*
...
*Why Precept, not convention:* Precept has 10 precedence levels, prefix operators (`not`, unary `-`), postfix operators (`.`, `(`), and non-associative comparisons. A Pratt parser expresses all of this in ~80 lines of dispatch logic plus a binding-power table. A recursive-descent expression parser would need 10+ mutually recursive methods with identical structure. *Rejected alternative: ANTLR or other parser generator. Generators produce opaque code that cannot participate in catalog-driven dispatch, and they impose a build-time dependency that the rest of the pipeline avoids.*
...
*Why Precept, not convention:* The language server calls `Parser.Parse` on every keystroke. A parser that fails on incomplete input forces the language server to special-case "no tree" scenarios throughout completions, hover, go-to-definition, and diagnostics. A resilient parser eliminates that entire class of defensive code. The MCP tools (`precept_compile`, `precept_inspect`) also consume the tree directly — they need a structurally coherent tree even when the author is mid-edit. *Rejected alternative: two-phase parsing — quick parse for tooling, full parse for compilation. This doubles the maintenance surface and risks semantic divergence between the two paths.*
...
*Why Precept, not convention:* Precept is a line-oriented policy language. The author writes declarations top-to-bottom; the runtime reads them the same way. A flat list matches the mental model: each line is a self-contained declaration with its own preposition scope. Nested AST blocks would impose a hierarchical structure that the language surface does not express. *Rejected alternative: scope-nesting parser that groups state-scoped declarations under their state node. This requires the parser to track "current scope" state, complicates error recovery when declarations appear out of order, and forces the type checker to unwrap nesting that the language never expressed.*
