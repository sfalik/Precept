...
The declared-bound value-safety codes are **`Proof`**: `NumericOverflow` (PRE0078) and `OutOfRange` (PRE0079) are produced by the proof engine, partitioned by subject category — a declared **default value** that violates a numeric bound is `OutOfRange`; a **computation** result (set-action or computed-field expression) that exceeds a field's bounds is `NumericOverflow`. `LengthBoundViolation` (PRE0135) covers string-length-bound violations on both set-actions and string defaults (including `notempty` as `minlength 1`); it is reachable on **non-literal** assignment paths (field/arg references, concatenation, conditionals, interpolation, member access, string functions), discharged against the RHS's static string-length interval — an unbounded source flowing into a capped field emits. The type checker stamps these as proof obligations rather than emitting them inline; `MaxPlacesExceeded` (PRE0067) remains type-owned (no compile-time precision proof yet).
