...
The declared-bound value-safety codes are **`Proof`**: `NumericOverflow` (PRE0078) and `OutOfRange` (PRE0079) are produced by the proof engine, partitioned by subject category — a declared **default value** that violates a numeric bound is `OutOfRange`; a **computation** result (set-action or computed-field expression) that exceeds a field's bounds is `NumericOverflow`. `LengthBoundViolation` (PRE0135) covers string-length-bound violations on both set-actions and string defaults (including `notempty` as `minlength 1`); it is reachable on **non-literal** assignment paths (field/arg references, concatenation, conditionals, interpolation, member access, string functions), discharged against the RHS's static string-length interval — an unbounded source flowing into a capped field emits. `CountBoundViolation` (PRE0136) is the cardinality sibling: it covers collection `mincount`/`maxcount` containment on every count-changing mutation (`add`/`remove`/`clear`) and the field `default [...]` literal, discharged against the post-mutation count interval the obligation carries (seeded from the governed band, narrowed by a `when C.count …` guard or routed reject row). It is an **obligation** diagnostic (§0.6 item 5 *unresolved* class, like the divisor obligation), not a *violation* assertion: an unprovable grow/shrink — a provable violation **or** a merely-unprovable case — emits a guard-naming message (overflow → `guard with when F.count < N`; underflow → `guard with when F.count > M`) rather than reporting a definite count. It is off the Gate-1 (no-emission) allow-list — emitted and test-referenced (Gate-2, alongside `LengthBoundViolation`). The type checker stamps these as proof obligations rather than emitting them inline; `MaxPlacesExceeded` (PRE0067) remains type-owned (no compile-time precision proof yet).
...
- *Process-topology* defects the compiler proves impossible per `philosophy.md`: `UnreachableState`, `StructuralSinkState`, `DeadEndState`, `RequiredStateDoesNotDominateTerminal`. (A no-exit state marked `terminal` is an intended endpoint, not a dead-end; an *un*marked reachable no-exit state is the "entity gets stuck" bug the philosophy forbids.)
- *Uninhabitability*: `ContradictoryRule` (PRE0155) and `UnsatisfiableRule` (PRE0159) — a definition whose declared rules admit no satisfying configuration. No valid entity can exist, so the definition is rejected rather than compiled into an engine that governs nothing.

The remaining structural diagnostics **report** (`Severity.Warning`) — they leave the entity inhabitable and are advisory signals, not prevention defects: rule/guard *redundancy* (`VacuousRule`, `TautologicalGuard`, `UnsatisfiableGuard` — an inert construct that governs or narrows nothing) and *dead-declaration / deliberate-prohibition* hygiene (`UnhandledEvent`, `AlwaysRejecting` transition rows, `StateAlwaysRejects`, `FieldNeverSet`). A deliberately-forbidden event and a redundant-but-harmless rule are supported designs, not errors.

This block-vs-report split is the **ruled severity model**; where a flipped code (`UnreachableState`, `StructuralSinkState`, `DeadEndState`, `RequiredStateDoesNotDominateTerminal`, `ContradictoryRule`, `UnsatisfiableRule`) still emits `Severity.Warning` in `Diagnostics.cs`, that is tracked drift — the code severity flip lands in a later implementation slice; this section states the target severity.

**Per-instance severity exists.** An earlier version of this section claimed Precept has no per-instance severity mechanism. That is inaccurate: `AlwaysRejecting` already carries `Severity.Error` on **construction** rows (`GraphAnalyzer.cs:783`, `... with { Severity = Severity.Error }`) while staying `Severity.Warning` on transition rows — one `DiagnosticCode` carrying two severities today. The severity families above are baseline-per-code assignments and need no per-instance override, but the mechanism is real and in use.

---

## DiagnosticCode Registry

`PRE0141 UnprovedAssignmentQualifierCompatibility` is the assignment companion to `PRE0114 UnprovedQualifierCompatibility`; both are **proof-stage** obligations. For an open-field `set`-action assignment, the type checker stamps an `AssignmentQualifierProofRequirement` (it owns the authoritative source-qualifier resolution) and the proof engine discharges it via guard-narrowing — emitting `PRE0141` only when unproved. (`PRE0141` formerly emitted at the type stage; it was re-staged when open-field qualifier narrowing landed, so a guard like `when X.currency == 'USD'` can discharge it. Field/arg defaults and computed-field expressions are likewise proof-owned — the type checker stamps the obligation for the proof engine to discharge. The sole remaining type-owned case is a `TypedConditional`-valued site, whose whole-value obligation cannot be sited per-branch.) Keep `PRE0114` for operand-pair obligations and `PRE0068` / `PRE0069` for definite mismatches.

The unproved messages name the **narrowed-vs-required** values: `PRE0141` appends a detail clause — *"the guard narrows it to 'X', which does not satisfy the required 'Y'"* (or *"no guard narrows it to the required 'Y'"* when no guard applies; the type-stage default/bounds path says *"the source's qualifier is not statically known"*). `PRE0114` and the open-period `PRE0113` render an open operand's guard-narrowed value with a `(from guard)` suffix (e.g. `EUR (from guard)`) instead of an opaque `unresolved`, so the author sees what the guard pinned the value to versus what the operation required.

`PRE0114`'s message is **domain-targeted**: it names the real-world mismatch and a concrete fix rather than compiler vocabulary — *"'A' (Currency: 'USD') and 'B' (Currency: 'EUR') measure different things and don't combine — align both to the same Currency, or insert an explicit conversion"*. The wording is axis-neutral so it reads naturally across the operand-pair cases the code emits (a currency mismatch in `A - B`, a dimension mismatch in `price × quantity`), naming both operands, both resolved qualifier values, and the axis. Both emission sites pass the same six arguments (left label, right label, axis, context clause, left qualifier value, right qualifier value).
