...
**`mincount`/`maxcount` participate in proof — discharged by a `when C.count …` guard.** Beyond the access-safety discharge above, the cardinality bounds are *carrier-discharged obligations* on every count-changing mutation: an in-place grow (`add`/`append`/`enqueue`/`push`/`put`/`insert`) into a `maxcount`-bounded field, or a shrink (`remove`/`removeAt`/`pop`/`dequeue`/`clear`) of a `mincount`-bounded field, must *prove* the count stays in band or the compiler emits `CountBoundViolation` (PRE0136) naming the guard. The carrier is a `when C.count < N` guard on a grow (or `when C.count > M` on a shrink), or a routed `when C.count >= N -> reject` sibling row — the same guard idiom as the non-empty `when C.count > 0`. The field `default [...]` carries the same obligation against its exact element count. (A `set C = [literal]` is *not* a count site — a list literal is legal only in `default`, so a literal RHS in a handler is type-rejected.) (An unguarded grow into a `maxcount` field, or an unguarded shrink of a `mincount` field, does not compile silently — it must name a guard, mirroring the divisor/length obligations.)
