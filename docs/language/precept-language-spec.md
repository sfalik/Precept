...
**Sequential proof flow (item 7) — partially implemented (corrected 2026-05-28).** A prior version of this table claimed item 7 was fully implemented; a probe disproved it (a guard fact survived a reassignment, proving a divide-by-zero "safe"). As of 2026-05-28: reassignment-**invalidation** is implemented for full-value-replacement actions (`set`/`clear`) in the guard-in-path strategy (numeric + presence obligations) — a guard fact about a field is dropped for obligations whose site follows a `set`/`clear` of that field. Two clauses remain follow-on work: (a) extending the same invalidation to the index-bounds, key-presence, and field-to-field guard strategies — these **still exhibit the over-prove (unsound) bug** on reassignment, because they don't yet consult the invalidation set (tracked as a follow-on); (b) the forward-propagation clause ("…before the new assignment's facts are stored") — re-establishing facts from the new value, including effect-aware collection-mutation reasoning (a grow like `insert`/`append` preserves `count > 0`; a shrink like `remove` may invalidate it). The fixed numeric+presence path is *sound* (it under-proves — rejects safe `set`-then-reestablish chains — rather than accepting unsound ones); the completeness of (b) and the soundness of (a) are the remaining work.
