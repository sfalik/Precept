...
Both directions are *sound by construction* (invalidation paths are pure rejections; grow-establishment proves only `count > 0`, which one added element guarantees). One bounded incompleteness remains: the grow/shrink model is boolean rather than a count interval, so `count >= 2 → dequeue → dequeue` under-proves the second dequeue (sound, conservative); exact-count interval tracking and membership-across-shrink are future refinements, not soundness gaps.
