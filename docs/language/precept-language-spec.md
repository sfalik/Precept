...
**Sequential proof flow (item 7) — invalidation implemented; forward-propagation pending (corrected 2026-05-28, completed 2026-05-29).** A prior version of this table claimed item 7 was fully implemented; a probe disproved it (a guard fact survived a reassignment, proving a divide-by-zero "safe"). Reassignment-**invalidation** is now implemented for full-value-replacement actions (`set`/`clear`) across **all** guard-narrowing strategies — guard-in-path (numeric + presence), index-bounds, key-presence, and field-to-field: a guard fact about a field is dropped for any obligation whose site follows a `set`/`clear` of that field, so the over-prove bug is closed for these strategies. One clause remains follow-on work: the forward-propagation clause ("…before the new assignment's facts are stored") — re-establishing facts from the new value, including effect-aware collection-mutation reasoning (a grow like `insert`/`append` preserves `count > 0`; a shrink like `remove` may invalidate it). The invalidation path is *sound* (it under-proves — rejects safe reassign-then-reestablish chains — rather than accepting unsound ones); the completeness of forward-propagation is the remaining work.
