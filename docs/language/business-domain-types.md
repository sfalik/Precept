...
- [Summary](#summary)
  - [The NodaTime alignment directive — extended](#the-nodatime-alignment-directive--extended)
- [Motivation](#motivation)
  - [The business-domain gap](#the-business-domain-gap)
  - [Before and After](#before-and-after)
  - [What happens if we don't build this](#what-happens-if-we-dont-build-this)
- [Backing Standards](#backing-standards)
  - [ISO 4217 — Currency codes](#iso-4217--currency-codes)
  - [UCUM — Units of measure](#ucum--units-of-measure)
  - [`decimal` backing — no `double` in the business chain](#decimal-backing--no-double-in-the-business-chain)
- [The `in` and `of` Qualification System](#the-in-and-of-qualification-system)
  - [`in` — specific unit](#in--specific-unit)
  - [`of` — category constraint](#of--category-constraint)
  - [Admission vs arithmetic](#admission-vs-arithmetic)
...
- [Compound Types and Dimensional Cancellation](#compound-types-and-dimensional-cancellation)
  - [The `/` ratio model](#the--ratio-model)
  - [The fixed-length boundary (D15)](#the-fixed-length-boundary-d15)
  - [Why `price` and `exchangerate` are named types, not generic `quantity` with compound units](#why-price-and-exchangerate-are-named-types-not-generic-quantity-with-compound-units)
- [Discrete Equality Narrowing](#discrete-equality-narrowing)
  - [The contract](#the-contract)
  - [Mechanism](#mechanism)
  - [Accessors per type](#accessors-per-type)
...
- [Period Extensions](#period-extensions)
  ...
  - [Default NodaTime basis (no `in` specified)](#default-nodatime-basis-no-in-specified)
  - [Lowering rules](#lowering-rules)
  - [Adding an existing period](#adding-an-existing-period)
  - [Period operations that produce a `period` (from temporal design)](#period-operations-that-produce-a-period-from-temporal-design)
- [Literal Forms](#literal-forms)
  - [Door 2 — Typed constant (`'...'`) with interpolation](#door-2--typed-constant--with-interpolation)
  - [Content validation](#content-validation)
  - [Integer requirement — scoped to temporal only](#integer-requirement--scoped-to-temporal-only)
  - [Scalar literal type resolution](#scalar-literal-type-resolution)
  - [Interpolation — any component, any position](#interpolation--any-component-any-position)
- [Field Constraints](#field-constraints)
  - [Magnitude types](#magnitude-types)
  - [Identity types](#identity-types)
  - [Constraint + `in`/`of` interaction](#constraint--inof-interaction)
- [Semantic Rules](#semantic-rules)
  - [Result-type algebra](#result-type-algebra)
...
- [Locked Design Decisions](#locked-design-decisions)
  ...
  - [D13a. String-form serialization — types own their own `Parse`/`ToString`](#d13a-string-form-serialization--types-own-their-own-parsetostring)
  - [D14. `in` is a uniform assignment constraint across all types](#d14-in-is-a-uniform-assignment-constraint-across-all-types)
  - [D15. Time-unit denominators use NodaTime vocabulary and cancel against `period` or `duration`](#d15-time-unit-denominators-use-nodatime-vocabulary-and-cancel-against-period-or-duration)
  - [D16. Business-domain magnitude types inherit Precept `decimal` semantics by default — domain identity justifies specific exceptions](#d16-business-domain-magnitude-types-inherit-precept-decimal-semantics-by-default--domain-identity-justifies-specific-exceptions)
  - [D17. `Precept.Types` namespace — no separate assembly split](#d17-precepttypes-namespace--no-separate-assembly-split)
  - [D18. CLR types are pure data records — all computation in executor modules](#d18-clr-types-are-pure-data-records--all-computation-in-executor-modules)
  - [D19. No qualifier inference on derivation operations](#d19-no-qualifier-inference-on-derivation-operations)
- [Relationship to Temporal Type System](#relationship-to-temporal-type-system)
...
- [Implementation Scope](#implementation-scope)
  ...
  - [Type checker changes](#type-checker-changes)
  - [Runtime engine changes](#runtime-engine-changes)
  - [Language server changes](#language-server-changes)
  - [TextMate grammar changes](#textmate-grammar-changes)
...
Time units (`s`, `min`, `h`, `d`) are excluded from the `quantity` category system because they belong to NodaTime's temporal types. Counting units (`each`, `box`, `case`, `pack`, `pallet`, `dozen`) live in the shared **count** dimension: they are registered with `DimensionVector.None` and normalize with factor-one semantics. That shared dimension class does **not** make them interchangeable or universally convertible. Cross-counting-unit operations — including addition (`qty_each + qty_box`), subtraction, comparison, same-match function calls, **and multiplication / division** when both operands are dimensionless with differing unit names — fail with PRE0137 `CrossCountingUnitOperation`, not PRE0071. PRE0071 is for different physical dimensions (for example `kg` vs `m`) and applies only to additive arithmetic; PRE0137 is for same-dimension count quantities whose explicit unit codes differ and have no universal UCUM conversion factor, and applies across the full qualifier-equivalence operator family. Conversion between counting units still requires explicit multiplication by a typed business conversion factor (e.g., `quantity in 'each/case'`).
...
public sealed class CurrencyCatalog
{
    ...
    // ISO 4217 amendment identifier, e.g., "2024-03"
    public string DataVersion { get; }
}
```

`CurrencyCatalog` is simpler than `UnitCatalog` because ISO 4217 is simpler than UCUM:
...
- **Tradeoff accepted:** UCUM conversion factors are carried as exact rationals (`UcumExactFactor` — a GCD-reduced `BigInteger` numerator/denominator plus a base-10 exponent; see [`catalog-system.md` § UCUM atom metadata](catalog-system.md)) and composed exactly through prefix and derived-unit reduction; the factor collapses to `decimal` only where it is applied to a magnitude, so conversion rounding occurs at the value boundary rather than in the factor itself. Authors who have `field Rate as number` and want to multiply by a `money` field must change the declaration to `as decimal` — the teachable error message explains why. This is a one-time migration cost that prevents silent precision loss in every subsequent operation.
...
### D17. `Precept.Types` namespace — no separate assembly split

- **What:** `Money`, `Quantity`, `Currency`, `UnitOfMeasure`, `MeasureDimension`, `UnitCatalog`, `CurrencyCatalog`, `Price`, and `ExchangeRate` live in the `Precept.Types` namespace within the existing `Precept` assembly. No separate NuGet package or assembly split.
- **Why:** These types are tightly coupled to the evaluator's internal representation. Separating them into a second assembly creates a circular dependency (evaluator needs `Precept.Types`; `Precept.Types` needs catalog loading which needs evaluator-adjacent infrastructure) or requires an awkward interface-extraction pass. The coupling is architectural: `CurrencyCatalog` is loaded at the same point as `UnitCatalog`, and both feed the type-resolution pipeline.
- **Alternatives rejected:** (A) Separate `Precept.Types` NuGet — circular dependency; breaks the sealed-class interning pattern for `Currency`. (B) Separate `Precept.Core` assembly for internal types only — over-engineering for the size of this codebase.
- **Tradeoff accepted:** Callers who want `Money`, `Quantity`, etc. must take a dependency on the full `Precept` assembly. Acceptable — catalog loading is startup-cost-only and the assembly is already a required dependency for the runtime.

### D18. CLR types are pure data records — all computation in executor modules

- **What:** `Money`, `Quantity`, `Price`, and `ExchangeRate` carry only construction, `ToString()`/`Parse()`, and value equality (auto-generated by `readonly record struct`). No arithmetic operators, no validation logic, no named computation methods. All computation lives in named executor modules (`MoneyOperations`, `QuantityOperations`, `PriceOperations`, `ExchangeRateOperations`) as `internal static` classes in `src/Precept/Runtime/Operations/`.
- **Why:**
  1. **No structural duplication.** The seven domain rules (same-currency guard, same-unit guard, D15 boundary, D16 exception table, D8 auto-conversion, precision enforcement, positive-rate invariant) would need enforcement in BOTH CLR type operators AND the evaluator pipeline under computation-on-types. Two enforcement points for the same rule must stay in sync manually — a maintenance trap.
  2. **C# operator limitations.** C# operators cannot take extra parameters. `Quantity.operator+(Quantity, Quantity)` requires same-unit enforcement, but D8 auto-conversion requires an `IUnitConversionSource`. The operator surface cannot be satisfied under computation-on-types.
  3. **Return-type ambiguity.** `Money / Money` returns `decimal` (same-currency ratio) but `Money / Quantity` returns `Price`. C# cannot return two types from one operator signature based on operand combinations, forcing named methods that bypass the operator surface.
- **Alternatives rejected:** (A) Computation-on-types with method injection — two enforcement points; see reason 1. (B) Computation-on-types with named methods only — abandons the operator surface; named methods cannot be called by the evaluator's opcode dispatch model. (C) Extension methods on CLR types — leaky public API surface; doesn't solve the two-enforcement-point problem.
