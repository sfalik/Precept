...
- [Summary](#summary)
  - [Canonical temporal parser](#canonical-temporal-parser)
  - [Temporal types as the gateway beyond primitives](#temporal-types-as-the-gateway-beyond-primitives)
  - [The NodaTime alignment directive](#the-nodatime-alignment-directive)
- [Motivation](#motivation)
  - [The temporal gap](#the-temporal-gap)
  - [Before and After](#before-and-after)
  - [What happens if we don't build this](#what-happens-if-we-dont-build-this)
- [NodaTime as Backing Library](#nodatime-as-backing-library)
  - [Why NodaTime, not the BCL temporal primitives](#why-nodatime-not-the-bcl-temporal-primitives)
- [Proposed Types](#proposed-types)
  ...
  - [`datetime`](#datetime)
- [Timezone Mediation — `.inZone(tz)` Dot Accessor](#timezone-mediation--inzonetz-dot-accessor)
  - [The single mediation operation](#the-single-mediation-operation)
  - [Why one operation, not three functions](#why-one-operation-not-three-functions)
  - [Strict hierarchy — no skip-level accessors (Decision #22)](#strict-hierarchy--no-skip-level-accessors-decision-22)
  - [DST ambiguity resolution (locked)](#dst-ambiguity-resolution-locked)
  - [Composability with existing operations](#composability-with-existing-operations)
  - [Temporal Quantity Construction — Typed Constants](#temporal-quantity-construction--typed-constants)
- [Literal Mechanism Architecture](#literal-mechanism-architecture)
  - [Context-born resolution — type from context, then content validation](#context-born-resolution--type-from-context-then-content-validation)
  - [Why two doors are sufficient](#why-two-doors-are-sufficient)
  - [Temporal types prove the framework](#temporal-types-prove-the-framework)
- [Semantic Rules](#semantic-rules)
  - [Type-interaction matrix](#type-interaction-matrix)
  - [Comparison rules](#comparison-rules)
  - [Cross-type arithmetic: what's NOT allowed (and why)](#cross-type-arithmetic-whats-not-allowed-and-why)
  - [Optional and default behavior](#optional-and-default-behavior)
- [Locked Design Decisions](#locked-design-decisions)
  - [1. NodaTime as the backing library for all temporal types](#1-nodatime-as-the-backing-library-for-all-temporal-types)
  - [2. Day granularity for `date`](#2-day-granularity-for-date)
  - [3. ISO 8601 as the sole format](#3-iso-8601-as-the-sole-format)
  - [4. Single-quoted delimiter is the typed constant delimiter — not temporal-specific (revised — locked 2026-04-15)](#4-single-quoted-delimiter-is-the-typed-constant-delimiter--not-temporal-specific-revised--locked-2026-04-15)
  - [5. No timezone on `date`, `time`, or `datetime`](#5-no-timezone-on-date-time-or-datetime)
  - [6. `instant` has no component accessors — restricted semantics (strengthened in v4)](#6-instant-has-no-component-accessors--restricted-semantics-strengthened-in-v4)
  - [7. `timezone` as a first-class type](#7-timezone-as-a-first-class-type)
  - [8. Timezone mediation via `.inZone(tz)` dot accessor (rewritten in v4)](#8-timezone-mediation-via-inzonetz-dot-accessor-rewritten-in-v4)
  - [9. Determinism is relative to the runtime environment](#9-determinism-is-relative-to-the-runtime-environment)
  - [10. `date + integer` is a type error — use `date + '{n} days'`](#10-date--integer-is-a-type-error--use-date--n-days)
  - [11. `date - date` returns `period` (not integer, not duration)](#11-date---date-returns-period-not-integer-not-duration)
  - [12. `period` is a surface type (field type AND expression type)](#12-period-is-a-surface-type-field-type-and-expression-type)
  - [13. `period` is full NodaTime Period — date AND time components (v2.1 revision)](#13-period-is-full-nodatime-period--date-and-time-components-v21-revision)
  - [14. No ordering on `period` — `==` and `!=` only](#14-no-ordering-on-period---and--only)
  - [15. Structural equality on `period` — `1 month != 30 days`](#15-structural-equality-on-period--1-month--30-days)
  - [16. Sub-day duration bridging — `time + duration` via thin type-checker translation](#16-sub-day-duration-bridging--time--duration-via-thin-type-checker-translation)
  - [17. Typed constant quantities — sole mechanism for temporal quantity construction (rewritten 2026-04-17)](#17-typed-constant-quantities--sole-mechanism-for-temporal-quantity-construction-rewritten-2026-04-17)
  - [18. Single-quoted typed constants — `'...'` as the typed constant delimiter (rewritten 2026-04-15, reframed 2026-04-15, updated 2026-04-17)](#18-single-quoted-typed-constants---as-the-typed-constant-delimiter-rewritten-2026-04-15-reframed-2026-04-15-updated-2026-04-17)
  - [19. Bare English naming — `date`, `time`, `datetime` (revised v4)](#19-bare-english-naming--date-time-datetime-revised-v4)
  - [20. Dot-vs-function rule — "If the value owns it, dot" (locked v4)](#20-dot-vs-function-rule--if-the-value-owns-it-dot-locked-v4)
  - [21. `inZone` as the sole timezone mediation operation (locked v4)](#21-inzone-as-the-sole-timezone-mediation-operation-locked-v4)
  - [22. Strict type hierarchy — no skip-level accessors (locked v4)](#22-strict-type-hierarchy--no-skip-level-accessors-locked-v4)
  - [23. `pin` eliminated — reverse navigation via dot chain (locked v4)](#23-pin-eliminated--reverse-navigation-via-dot-chain-locked-v4)
  - [24. Instant restricted semantics — comparison, duration arithmetic, and `.inZone(tz)` only (locked v4)](#24-instant-restricted-semantics--comparison-duration-arithmetic-and-inzonetz-only-locked-v4)
  - [25. `date + time → datetime` as the sole component construction path (locked v4)](#25-date--time-→-datetime-as-the-sole-component-construction-path-locked-v4)
  - [26. Period component constraints — `of 'date'` and `of 'time'` (locked v4, updated v6)](#26-period-component-constraints--of-date-and-of-time-locked-v4-updated-v6)
  - [27. No `datetime + duration` — context-dependent resolution to `period` instead (locked v4)](#27-no-datetime--duration--context-dependent-resolution-to-period-instead-locked-v4)
  - [28. Temporal typed constant quantities require integer values (locked v4, updated v5)](#28-temporal-typed-constant-quantities-require-integer-values-locked-v4-updated-v5)
- [Double-Quote Alternative Analysis](#double-quote-alternative-analysis)
  - [The compiler CAN resolve it in ~99% of cases](#the-compiler-can-resolve-it-in-99-of-cases)
  - [Why it was rejected](#why-it-was-rejected)
  - [The choice-type precedent](#the-choice-type-precedent)
  - [Conclusion](#conclusion)
- [Design Challenges — Resolution](#design-challenges--resolution)
  - [Challenge 1: `DueDate + MealsTotal` compiles because `date + number → date`](#challenge-1-duedate--mealstotal-compiles-because-date--number-→-date)
  - [Challenge 2: `date + 2.5` should reject fractional offsets](#challenge-2-date--25-should-reject-fractional-offsets)
  - [Challenge 3: `optional + default` prohibition](#challenge-3-optional--default-prohibition)
  - [Challenge 4: `date - date` result type](#challenge-4-date---date-result-type)
- [Dependencies and Related Issues](#dependencies-and-related-issues)
- [Explicit Exclusions / Out of Scope](#explicit-exclusions--out-of-scope)
  - [Collection inner types](#collection-inner-types)
- [Implementation Scope](#implementation-scope)
  ...
  - [MCP Tools](#mcp-tools)
  - [Samples, Documentation, Tests](#samples-documentation-tests)
- [Forward Design: Temporal Types as the Gateway Beyond Primitives](#forward-design-temporal-types-as-the-gateway-beyond-primitives)
  - [Why this matters now](#why-this-matters-now)
  - [Natural extension domains](#natural-extension-domains)
  - [Unit resolution model — three validation scopes](#unit-resolution-model--three-validation-scopes)
  - [Design implications for temporal decisions](#design-implications-for-temporal-decisions)
  - [Three design levels for quantity types](#three-design-levels-for-quantity-types)
  - [Standards landscape for future reference](#standards-landscape-for-future-reference)
  - [What exists today (gap analysis)](#what-exists-today-gap-analysis)
...
**What changed in v5:** The literal mechanism architecture transitioned from a three-door model to a **two-door model** (see [Literal System](../compiler/literal-system.md)):
...
**Overflow note (deferred, post-MVP — same decision as integer overflow):** `instant ± duration` can push the result outside the representable range (`-9998-01-01T00:00:00Z` to `9999-12-31T23:59:59Z`), which faults at runtime. How this is handled is **not settled**: it is deferred under the arithmetic overflow model in [primitive-types.md](primitive-types.md#open-questions--implementation-notes), and the temporal arm follows whichever way that decision goes.

Two earlier claims in this note were wrong and are corrected here. It is **not** "an accepted edge case" — that reads as a ruling, and no ruling has been made. And the compiler does **not** need runtime field values to catch it: the proof engine bounds operands from their declared constraints, which is how every other arithmetic fault obligation is discharged. The real obstacle is narrower and worth recording for whoever rules the deferral — temporal point types reject `min`/`max` constraints today, so there is no field-constraint premise to bound a temporal operand with. Closing this arm needs a premise source: bound vocabulary on temporal types, guard-only closure, or a declared representable-range axiom.
...
The v3 proposal had three standalone conversion functions: `toLocalDate(instant, tz)`, `toLocalTime(instant, tz)`, `toInstant(date, time, tz)`. Three locked decisions collapsed these to one:
...
**Status:** Locked (Decision #17 — rewritten 2026-04-17). Quantities enter through Door 2 as typed constants. See [Literal System](../compiler/literal-system.md) for the canonical two-door model.
...
Unit names: `days`, `months`, `years`, `weeks`, `hours`, `minutes`, `seconds`. These are **not** language keywords — they are validated strings inside `'...'`. See [Literal System](../compiler/literal-system.md) § Quantity unit names.
...
**Design rationale:** "The author IS thinking about the details — they wrote `instant + '3 days'`, which means '3 fixed 24-hour days on the timeline.' The context makes the intent unambiguous."
...
- **Why:** NodaTime alignment directive: *"No obscurity, expose NodaTime."* 10 period fields across 7 samples are `integer` surrogates. The integer loses the unit; the period carries it. NodaTime provides full factories, operators, equality, and serialization for `Period`.
...
- **Scope:** This restriction applies to **temporal** unit names (`days`, `months`, `years`, `weeks`, `hours`, `minutes`, `seconds`) inside typed constants. The typed constant grammar mechanism is a general language expansion joint (noted in the proposal's "Expansion Joint" section and in [Literal System](../compiler/literal-system.md)). Future non-temporal quantity typed constants (e.g., currency, measurement units) may support non-integer values if their backing types accept them.
...
- String interpolation: evaluate `{expr}` inside `"..."`, coerce to string per coercion table in [Literal System](../compiler/literal-system.md).
...
Temporal types are not just a feature — they are the **gateway** to Precept's evolution beyond primitive types. The literal mechanisms established in this proposal — the single-quoted typed constant delimiter (`'...'`) with `{expr}` interpolation — are the language's **expansion joints** for all future non-primitive types. The canonical design for these mechanisms lives in [Literal System](../compiler/literal-system.md). Every non-primitive value in Precept enters through one of two doors: `"..."` (string) or `'...'` (typed constant). When a future type is proposed, the question is "which door?" — not "does it need new syntax?"
