...
**UCUM atom metadata.** Each UCUM atom (`UcumAtom`) carries the per-unit facts consumers derive from rather than recompute: its `DimensionVector`, its exact-rational scale-to-base factor (`Scale`, a `UcumExactFactor` — a reduced rational with a base-10 exponent), an optional affine `AffineOffset` (the °C/°F absolute-reading offset), and `ScaleIsRational`. `ScaleIsRational` records whether `Scale` is an exact rational suitable for amount-conversion: it is `false` for units whose scale-to-base is irrational — the plane-angle family (`rad`/`deg`/`gon`/`'`/`''`, whose reduction passes through `[pi]` and is stored only as a rational *approximation*) and logarithmic units (`B`/`Np`, and the `dB` they back, defined by a `lg`/`ln` UCUM function rather than a multiplicative scale). It is `true` for everything else, including the affine units `Cel`/`[degF]` — their amount-conversion scale (e.g. `5/9`) is an exact rational; the affine offset is a separate absolute-reading concern. The flag is computed structurally at catalog load (the `[pi]`/`lg`/`ln` signal, propagated transitively through derived units) and consumed for cross-unit conversion surfacing (`price × quantity` cancellation labels its conversion exact vs. approximate from this flag) — derived from the registry, never recomputed by consumers.
...
| **ProofEngine** | Catalog-declared obligations | Full design documented in `docs/compiler/proof-engine.md`. `ProofRequirement[]` on `BinaryOperationMeta`, `FunctionOverload`, `TypeAccessor`, and `ActionMeta` carry all proof obligations as metadata. `ProofRequirements.GetMeta(kind)` dispatches obligation instances. `ValueModifierMeta.ProofDischarges` enables catalog-driven modifier-proof strategy (CC#5 resolved). Sequential-proof-flow reasoning (spec § 0.6 item 7) reads `ActionMeta.Effect` (`ActionEffectClass`) to classify each action as full-replacement / collection grow / shrink — the engine never switches on `ActionKind` identity to decide reassignment-invalidation or count forward-propagation. |
