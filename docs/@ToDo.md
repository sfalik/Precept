...
- ✅ [Phase 7](McpServerImplementationPlan.md#Phase-7-Agent-Plugin-Structure--MCP-Packaging): created `tools/Precept.Plugin/` with `.claude-plugin/plugin.json` (Claude format required for `${CLAUDE_PLUGIN_ROOT}` expansion), dev `.mcp.json`, moved launcher to `tools/scripts/`, removed `Precept Dev` from `.vscode/mcp.json`, verified plugin loads via `chat.pluginLocations` on VS Code Insiders.
