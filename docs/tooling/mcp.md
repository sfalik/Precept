...
**Diagnostic code:** `DiagnosticCode.McpToolInternalError` = 149 (PRE0149). Stage: `DiagnosticStage.Tooling` — the wrapper error originates outside any compile-pipeline stage. Consumers that filter or count by stage should expect this code under the `Tooling` classification.

> **Per-stage diagnostic counts.** `precept_compile`'s reported type-error count derives from diagnostics whose `Stage == Type`. Name-resolution and duplicate-declaration diagnostics are now `Bind`-owned (and `NoInitialState` is `Graph`-owned), so those no longer count toward the type-error total — they surface under their own producing-component stage instead. The declared-bound value-safety codes `NumericOverflow` (PRE0078) and `OutOfRange` (PRE0079) are now `Proof`-owned (relocated from `Type`), so they shift from the type-error total into the proof-stage total. New proof-stage diagnostics also appear for previously-silent declared-bound cases: a string field/arg **default** below `minlength` (or `notempty` on an empty string) now emits `LengthBoundViolation` (PRE0135), and a **computed numeric field** whose result exceeds its declared bounds now emits `NumericOverflow` — both a correctness gain. A collection **string element bound** (`queue of string maxlength 200`) likewise surfaces through `precept_compile`: an element write-site (`enqueue`/`add`/`push`/`append`/`insert`/`put`) whose source cannot be proven within the element length bound emits `LengthBoundViolation` (PRE0135), and a typing mismatch (`set of integer maxlength 5`) emits `InvalidModifierForType` (PRE0033). No new diagnostic code or compile-DTO field is added — the element bound rides the existing diagnostics-plus-obligations channel.
