...
> **Per-stage diagnostic counts.** `precept_compile`'s reported type-error count derives from diagnostics whose `Stage == Type`. Name-resolution and duplicate-declaration diagnostics are now `Bind`-owned (and `NoInitialState` is `Graph`-owned), so those no longer count toward the type-error total — they surface under their own producing-component stage instead.
