...
- Compile-time checking — unreachable states, type errors, null-flow violations, and divisor safety caught before runtime; divisor proof covers compound operands (e.g., `Y / (A + 1 - B)`), sum-on-RHS rules (e.g., `rule Total > Tax + Fee`), computed-field intermediaries (e.g., `set Net = Gross - Tax; Y / Net`), and transitive ordering chains (e.g., `rule A > B`, `rule B > C` proves `Y / (A - C)`)
