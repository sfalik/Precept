...
internal static class PreceptLegacyParser
{
    ...
    public static PreceptDefinition Parse(string text)
    {
        ...
        while (i < lines.Length)
        {
            ...
            if (topLevelRuleMatch.Success)
            {
                ...
                try { ruleExpr = PreceptLegacyExpressionParser.Parse(ruleExprText); }
                catch (InvalidOperationException ex)
        ...
        string? fieldRuleOwner = null)
    {
        ...
        try { parsed = PreceptLegacyExpressionParser.Parse(expressionText); }
        ...
        IReadOnlyList<PreceptField> dataFields)
    {
        ...
        {
            try { whenAst = PreceptLegacyExpressionParser.Parse(whenText); }
            catch (InvalidOperationException ex)
        ...
        IReadOnlyList<PreceptField> dataFields)
    {
        ...
        try { guardAst = PreceptLegacyExpressionParser.Parse(guardExpression); }
        ...
        PreceptCollectionKind requiredKind)
    {
        ...
        try
        {
            var expression = PreceptLegacyExpressionParser.Parse(expressionText);
    ...
    private static object? ParseFieldDefaultLiteral(string text, PreceptScalarType type, bool isNullable, string fieldName, int lineNumber)
    {
        ...
        try
        {
            parsedExpression = PreceptLegacyExpressionParser.Parse(text);
        }
        catch (InvalidOperationException ex)
        {
