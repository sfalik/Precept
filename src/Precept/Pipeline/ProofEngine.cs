...
public static partial class ProofEngine
{
    ...
    ///    Set actions on bounded decimal/number fields generate IntervalContainmentProofRequirement to ensure
    ///    the assigned expression's value interval fits within the field's declared bounds [min..max].
    ...
    private static void WalkActions(ImmutableArray<TypedAction> actions, ObligationContext ctx, List<ProofObligation> obligations, SemanticIndex semantics)
    {
        foreach (var action in actions)
        {
            ...
            // For Set actions on bounded decimal/number fields: generates interval containment obligations.
            // This ensures the value being assigned stays within the field's declared bounds.
