...
- **Invoked completion path never appends a closing quote (2026-05-30):** The invoked path (`IsInsideTypedConstantToken` guard) only fires when the cursor is inside an existing typed-constant token. The token always has a closing `'` — either auto-close or explicit. Always pass `appendClosingQuote: false` there. The trigger-character `'` path is the only path that may need to append a closing quote, and it guards correctly with `!IsEmptyTypedConstantToken(...)`.
- **Test `InsertText` not just labels for typed-constant completions (2026-05-30):** Tests that only check item labels can miss `InsertText` regressions. For any test that validates quote-handling, assert `InsertText` (or `NotEndWith("'")`) explicitly. The mismatch between what labels show and what `InsertText` inserts is where double-quote bugs hide.
...
- **Interpolated `price in '{Field}'` currently threads as currency-only metadata (2026-05-17T07:58:11.045-04:00):** `TypeChecker.MapInterpolatedQualifier` resolves `QualifierAxis.PriceIn` with `expectedType: Currency` and emits `DeclaredQualifierMeta.Currency`, not `CompoundPrice`. Tooling should treat simple interpolated `in` qualifiers as `{CurrencyField}/${2:unit}` unless a literal `currency/unit` pair is actually declared; whole-pair prefills still come only from literal compound qualifiers.
- **Event modifier completion needs a post-`)` boundary guard (2026-05-17T08:46:26-04:00):** `TryGetCurrentEventArgName(...)` can still recover the last arg after the cursor has moved past the closing `)` of an event declaration. Guard `GetModifierDomain(...)` with `IsAfterEventArgumentList(...)` so `event Name(Arg as type) ` stays on the event-modifier lane and offers `initial` instead of value modifiers.
