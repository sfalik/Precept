...
- frank-25: String holes in interpolated typed constants should be excluded entirely. The proof engine treats typed constant fields as opaque (unfoldable in satisfiability, no content inspection). Three of five proof strategies (Declaration Attribute, Guard in Path, Qualifier Compatibility) gain power when string holes are excluded — qualifier compatibility goes from unprovable to fully provable because `DeclaredQualifiers` are resolvable from typed field declarations but not from string values. Zero samples use `currency`/`unitofmeasure` field types; the string-carrying-currency pattern (`fee-schedule.precept`'s `CurrencyCode as string`) is a design smell. Excluding string eliminates 26 compatibility-table rows, the runtime-deferred-check philosophy exception, and runtime content validation for interpolated results. The annotation alternative (`'{x:currency}'`) adds unnecessary complexity for a non-existent use case.
