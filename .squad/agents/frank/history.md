...
- **Scope:** Six types (`date`, `time`, `instant`, `duration`, `timezone`, `datetime`), three conversion functions (`toLocalDate`, `toLocalTime`, `toInstant`), calendar constructor functions (`months`, `years`, `weeks`), duration constructor functions (`hours`, `minutes`, `seconds`). NodaTime as backing library for all.
- **Supersedes #26** (date-only proposal). The `date` section incorporates all of #26's design plus NodaTime backing, `integer`-only arithmetic, and month/year arithmetic.
- **Open options presented (not pre-decided):** `duration` as field type vs expression-result only; `datetime` inclusion vs deferral; `zoneddatetime` minimal composite inclusion vs deferral; `nullable + default` for temporal fields (George's Challenge #3); DST resolution strategy (lenient vs strict); `annualdate`/`yearmonth`/`daterange` inclusion vs deferral.
- **All four George challenges resolved:** #1 (`DueDate + MealsTotal` type error — no `date + <non-duration>` is valid), #2 (fractional AND integer offsets both rejected), #3 (presented as open option), #4 (`date - date → duration`).
- **11 locked decisions** with full 4-point rationale format: NodaTime adoption, day granularity, ISO 8601, constructor form, no timezone on local types, instant no component accessors, timezone as type, conversion function explicit params, determinism model, explicit-duration-only arithmetic, date - date → duration.
- **Comprehensive cross-type interaction matrix** defining all valid operations and all type errors.
- **Full implementation scope** across parser, type checker, evaluator, runtime, grammar, language server, MCP tools, samples, docs, and tests.
- Committed: `proposal: unified temporal type system — NodaTime-aligned, complete design`

### 2026-04-13 — Eliminate implicit temporal arithmetic (Shane's directive)
- Shane's directive: "date + integer is implicit." Applied across the entire temporal proposal.
- **Decision #10 revised:** `date + integer` → `date + days(n)`. All temporal arithmetic now requires explicit duration constructors (`days()`, `months()`, `years()`, `weeks()`, `hours()`, `minutes()`, `seconds()`). `date + 2` is a type error.
- **Decision #11 revised:** `date - date → integer` → `date - date → duration`. Results carry unit semantics.
- Added `days(n)` constructor function. Unified all seven constructors under "Duration constructor functions" at the DSL surface.
- Updated all code examples, cross-type matrix, George's challenge resolutions (#1, #2, #4 all strengthened), ACs, and implementation scope.
- Decision filed: `.squad/decisions/inbox/frank-explicit-duration-arithmetic.md`

### 2026-04-13 — Timezone type storability analysis (response to Shane's "define storable" challenge)
- Shane's two-word challenge ("Well define storable") exposed a self-contradiction: the enterprise analysis claimed "entity data shouldn't carry timezone identity" while the proposed architecture stored timezone identity as `field IncidentTimezone as string notempty`.
- **Position change: `timezone` type proposed.** The `date`-over-`string` argument (encoding X as strings loses type safety) applies to timezones with equal force. `field IncidentTimezone as timezone` validates IANA identifiers at compile time (literals) and runtime (event args). Conversion functions accept `timezone` instead of `string`, closing the type-safety gap at the call boundary.
- **Position change: ZonedDateTime Fatal rating DOWNGRADED to Deferred.** Shane's minimal composite definition (instant + timezone, comparison by instant, `.instant`/`.timezone` accessors only, no component accessors, no arithmetic) eliminates every concern that drove the Fatal rating. The minimal composite is safe — the remaining question is cost/benefit, not safety. Type system cost (constructor syntax, composite accessors, null/default semantics, collection behavior) is significant; co-assignment enforcement is the primary benefit over two separate fields.
- **Self-contradiction acknowledged:** "Entity data should not carry timezone identity" was wrong as stated. The architecture always stored timezone data. The actual boundary is about the *type system* — whether timezone data is typed or untyped — not about whether it exists in the entity.
- **Phasing revised:** Phase 1 = `timezone` type ships with `instant` and conversion functions. Phase 2 = minimal `zoneddatetime` composite, demand-gated on enterprise adoption evidence.
- Analysis written to `research/language/expressiveness/timezone-type-storability-analysis.md`.
- Decision filed: `.squad/decisions/inbox/frank-timezone-storability.md`

### 2026-04-13 — Temporal type strategy (synthesis of NodaTime discussion arc)
- Wrote the unified temporal type strategy document: `research/language/expressiveness/temporal-type-strategy.md`.
- **Governing principle:** NodaTime and Precept share the same philosophy — explicit behavior over implicit behavior. Each temporal type earns its place by eliminating a specific ambiguity that would otherwise exist if the data were encoded as a weaker type.
- **11 types assessed.** 6 accepted across 3 phases: `date` (Phase 1), `instant`/`duration`/`time`/`timezone` (Phase 2), `datetime` (Phase 3). 3 deferred with real use cases (`annualdate`, `yearmonth`, `zoneddatetime` minimal composite). `Period` is internal mechanism, not a DSL type.
- **Determinism model reframed.** Determinism is about explicit inputs, not absent dependencies. The TZ database is an explicit input to conversion functions — visible in the expression, typed at the call boundary. Same category as .NET version and NodaTime version.
- **Impact on #26:** Backing type → `NodaTime.LocalDate`. `date + integer` (not `number`). Month/year arithmetic included in Phase 1 (corpus evidence: 10 period fields across 7 samples). 6 new proposals needed for Phase 2/3.
- Updated research language README with full temporal research trail (all 6 documents + 2 supporting references).
- Decision filed: `.squad/decisions/inbox/frank-temporal-strategy.md`

### 2026-04-13 — Enterprise multi-timezone analysis (response to Shane's ZonedDateTime challenge)
- Shane challenged the "hosting layer handles timezone conversion" argument with enterprise multi-timezone scenarios: filing deadlines at midnight local time in a jurisdiction that varies per entity instance, business-hours rules in per-customer timezones, SLA windows in service-region timezones.
- **Position change: timezone *conversion functions* are viable.** The "hosting layer handles it" argument breaks down when the timezone is entity data inside a business rule. Without timezone operations, the rule must be split between the contract and the hosting layer — violating "one file, complete rules."
- **Middle ground identified:** `toLocalDate(instant, string) → date`, `toLocalTime(instant, string) → time`, `toInstant(date, time, string) → instant`. No new types — these functions bridge existing types using an explicit timezone string parameter. `ZonedDateTime` as a *type* remains excluded.
- **Determinism argument revised:** Was applying a stricter standard to timezone operations than to everything else. The TZ database is already in the dependency tree via NodaTime. Timezone conversion functions are deterministic given the same runtime environment (including TZ database version) — the same standard that applies to all other dependencies. Previous argument was inconsistent.
- **Philosophy gap confirmed real:** Precept's target domains (insurance, healthcare, finance) are exactly the domains with timezone-dependent compliance rules. If the contract can't express those rules, the "one file, complete rules" claim has a structural gap in the primary market.
- **`ZonedDateTime` as a field type still excluded, Fatal rating stands.** Entity data should not carry timezone identity. Store instants, store timezone identifiers as strings, convert in expressions. The type-level exclusion is unchanged.
- **Phasing:** Timezone conversion functions should NOT ship with initial `instant` type. Phase: v2 = instant with comparison, v2.x/v3 = timezone conversion functions gated on enterprise demand.
- Analysis written to `research/language/expressiveness/enterprise-timezone-analysis.md`.
- Decision filed: `.squad/decisions/inbox/frank-timezone-reconsideration.md`

### 2026-04-13 — NodaTime feasibility analysis for Precept temporal types
- Produced comprehensive feasibility analysis at `research/language/expressiveness/nodatime-precept-alignment.md`.
- **Recommendation: ADOPT NodaTime** as the backing type library for Precept's temporal types (`LocalDate` for `date`, `LocalTime` for future `time`, `Period` for month/year arithmetic).
- **Philosophy alignment:** Near-perfect. NodaTime's "force you to think about what your data means" principle maps directly to Precept's prevention guarantee. Type separation (date vs time vs duration vs timezone) maps to Precept's determinism guarantee.
- **Directly resolves 2 of 4 George challenges on #26:** Challenge 2 (fractional day rejection — `PlusDays(int)` enforces integer) and Challenge 4 (`date - date → integer` confirmed by `Period.Between`).
- **Key architecture decision:** NodaTime is an implementation dependency, not a DSL surface dependency. DSL author writes `date`; runtime stores `NodaTime.LocalDate`. Same pattern as `decimal` → `System.Decimal`.
- **Proposed mapping:** `date` → `LocalDate` (v1), `time` → `LocalTime` (v2), `months()/years()` → `Period` functions (v2), `datetime` → `LocalDateTime` (v3). `Instant`, `ZonedDateTime`, `Duration` excluded permanently — violate determinism/one-file-completeness.
- **Risks assessed as low:** NodaTime is MIT-licensed, stable 14 years, serialization solved by companion package, API leakage manageable via transitive dependency acceptance.
- **Philosophy gap flagged:** NodaTime enables capabilities (calendar-system-aware arithmetic, sub-day temporal operations) that `docs/philosophy.md` does not describe. Flagged for Shane — not self-resolved.
- Decision filed: `.squad/decisions/inbox/frank-nodatime-alignment.md`

### 2026-04-13 — Instant and ZonedDateTime reconsideration (response to Shane's counterarguments)
- Shane challenged the blanket `Instant` exclusion and `ZonedDateTime` "Fatal" rating from the NodaTime feasibility analysis with three counterarguments: event timestamps as readonly context (SLA patterns), audit trail fields (HIPAA/SOX compliance), and the "store as Instant, display as local" NodaTime pattern.
- **Position change: `Instant` reconsidered as VIABLE.** Original analysis conflated comparison (deterministic — just nanosecond comparison) with component extraction (non-deterministic — requires timezone). These are separable. NodaTime's own `Instant` type has no component accessors, proving the separation is intentional.
- **Design boundary:** `instant` with comparison operators + duration arithmetic, NO component accessors (`.year`, `.month`, `.day` are compile errors). Deterministic by construction — same principle as NodaTime's type design.
- **`ZonedDateTime` exclusion UPHELD, Fatal rating stands.** Shane's "TZ database is operational" argument weakens Precept's structural guarantee to a convention-based guarantee. The "store as Instant, display as local" pattern proves the correct architecture has no ZonedDateTime in the domain contract.
- **Key concession:** The SLA/compliance use cases (72-hour filing windows, escalation SLAs, audit ordering) are real, appear in 5+ surveyed domains, and without `instant` authors would encode timestamps as numbers — the same type-safety failure the `date` type was designed to prevent.
- **Proposed roadmap:** `instant` at v2 (with `time` and month/year arithmetic), `duration` as expression-result type shipping with `instant`, new proposal issue needed.
- Analysis written to `research/language/expressiveness/instant-zoneddatetime-reconsideration.md`.
- Decision filed: `.squad/decisions/inbox/frank-instant-reconsideration.md`
...
### 2025-07-25 — Full NodaTime Alignment (Period/Duration Split)
- **Directive:** Shane: "NodaTime has already solved this hard problem. I see no reason to diverge."
- Applied full NodaTime alignment to the temporal type system proposal, closing all 5 divergences identified in the comparison analysis.
- **Period/duration split:** 8 temporal types (not 7). Calendar constructors (`days`, `months`, `years`, `weeks`) → `period` (backed by `NodaTime.Period`). Timeline constructors (`hours`, `minutes`, `seconds`) → `duration` (backed by `NodaTime.Duration`). Cross-type arithmetic (`period + duration`) is a type error.
- **ZonedDateTime calendar arithmetic forbidden:** `zdt + days(n)`, `+ period`, etc. are type errors. Users decompose via `.date`, do calendar arithmetic on `localdate`, then reconstruct. Prevents silent DST bugs.
- **LocalTime subtraction allowed:** `localtime - localtime → period`.
- **Sub-minute arithmetic:** `+ seconds(n)` added to `localtime`, `localdatetime`, `zoneddatetime`.
- **ZDT multi-dimensional comparison:** Equality checks instant → local datetime → timezone ID. Two ZDTs at same instant in different zones are NOT equal. Ordering is by instant only.
- Updated: all per-type sections, type-interaction matrix, comparison rules, cross-type error table, default behavior, constructor functions, implementation scope, locked decisions 10+11, George's Challenge 4, acceptance criteria.
- **Tool lesson:** `replace_string_in_file` cannot match Unicode em-dashes/smart quotes. Use Python scripts for sections with those characters. NEVER use PowerShell `WriteAllLines` — it corrupts UTF-8 encoded Unicode.
- File: `research/language/expressiveness/temporal-type-system-proposal.md`
