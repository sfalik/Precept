...
- EvaluatorDesign.md Track B design doc (2026-04-19): Drafted `docs/EvaluatorDesign.md` (~650 lines) as the canonical design for the expression evaluator's target state after issue #115. Key architectural choices: (1) three-lane numeric model (integer/`long`, decimal/`decimal`, number/`double`) with lane identity = C# runtime type — no metadata tags; (2) decimal lane closure guarantee — homogeneous decimal operations never pass through `double` intermediary; (3) context-sensitive literal typing with non-ambiguous inference invariant — fractional literals resolve from expression context, no suffix syntax; (4) `round(number, places) → decimal` as sole explicit approximate→exact bridge; (5) `double → decimal` rejected at runtime/API boundary — external callers must provide `decimal`-typed values; (6) `.count`/`.length` return `long` (integer), string slicing requires integer args — no silent truncation; (7) proof engine stays `double`-backed (`NumericInterval` widening is conservative/sound). Structured after ProofEngineDesign.md: philosophy-rooted principles, current state analysis (10 critical findings), architecture, lane integrity rules, design decisions (DD1–DD9 with rationale/alternatives/tradeoffs), test obligations, deferred items. The current state analysis documents the root cause: evaluator was built with `double` as universal numeric type before the three-type system was specified. The `long` integer lane was retrofitted correctly; the `decimal` lane was scaffolded but never plumbed through evaluator/collection/runtime paths.
