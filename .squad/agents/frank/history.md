...
- D3's closed-world access default (read unless explicitly opened to write) is not just a convenience choice — it is a structural safety property aligned with Principle 1 (prevention). Inverting it to write-by-default changes the omission failure mode from safe (locked) to unsafe (exposed). The corpus confirms the calibration: stateful precepts average 0–2 write declarations covering a small minority of (field, state) pairs; the overwhelming majority of pairs are correctly protected by D3's silent default. Proposals to reduce access-declaration verbosity should explore narrower mechanisms (`write all except`) rather than inverting the default. Decision written 2025-07-14.
- Computed fields' implicit readonly-ness is naturally aligned with D3's read default but inconsistent with a write default. Any access-default proposal must account for the stored/computed field asymmetry — a write default creates a hidden exception for computed fields that undermines inspectability.
- The "nothing crosses the boundary" claimin combined-design-v2 §1–§2 is overclaimed. Descriptors already carry source lines, expression text, constraint metadata, and scope targets into the runtime — that is analysis knowledge in lowered form. The real boundary is lifecycle and dependency direction: runtime types do not reference compile-time artifact types, but runtime shapes carry selected semantic knowledge that lowering transforms. The doc should describe what actually crosses and why, not assert a purity it demonstrably does not have.
...
- Boundary claim corrected, readability/genre fixes applied, assertions moved to doc spine, no content dropped. All changes logged and inbox decisions merged.
...
- Full philosophy-rooted rewrite of combined-design-v2. The previous version organized around the compiler/runtime boundary as its central concern — treating the "split" as the problem and spending energy defending it. The rewrite roots the entire document in `docs/philosophy.md` and Precept's structural guarantee promise. The new spine: §1 what Precept promises, §2 architectural approach (catalog-driven, purpose-built, unified pipeline), §3 pipeline overview, §4–§10 per-stage design (each opens with "how it serves the guarantee"), §11 runtime surface and operations (structured outcomes, constraint query, inspection), §12 LS integration, Appendix A implementation status. All technical content preserved — pipeline stages, per-stage contracts, descriptor types, constraint evaluation matrix, proof/fault chain, LS consumption matrix, action family, activation indexes. Removed: the "split as problem" framing, "structural tension" opening, enumerated architectural commitments list (replaced by narrative that makes the same points in context), §5.x numbering (replaced by named sections), defensive boundary language. The document now reads as a unified design explanation rather than a defense of two separate systems.

### 2026-06-03 — Full design review of combined-design-v2 (soundness, completeness, innovation)

- **Verdict: APPROVED-WITH-CONCERNS.** The architecture is sound and the philosophy-rooted framing is correct. Three concerns require resolution before the doc drives per-component design.
- **Proof coverage is the highest-risk unknown.** The four bounded proof strategies are well-chosen but their coverage against real sample obligations is unstated. If most obligations require cross-field reasoning, the strategy set is too narrow. A coverage analysis against the 20 sample files is needed before implementation.
- **Parser error recovery is a design decision, not an implementation detail.** The doc promises recovery-aware `SyntaxTree` but never specifies the recovery algorithm. LS quality depends on this — it must be locked before the parser is built.
- **The executable model shape is unspecified.** "Lowered expression nodes" could mean a tree interpreter or a flat evaluation plan. For Precept's closed expression language, a flat slot-addressed evaluation plan is the correct model — it's predictable-time, inspectable, and cache-friendly. The doc should commit to this and explicitly reject tree-walking.
- **Innovation finds:** (1) Initial-state satisfiability should be a compile-time guarantee — the research base already supports it. (2) A `ConstraintInfluenceMap` (constraint → contributing fields) lowered into the runtime would make AI inspection structurally superior to anything in the category. (3) A contract digest hash would enable definition-change detection and ground the migration story. (4) Structured "why not" explanations in constraint violations would transform MCP from status reporting to causal reasoning.
- **Completeness gaps flagged:** no incremental compilation model, no `Version` serialization contract, no definition versioning/migration story, no observability hooks. Each shapes downstream design if addressed now; each is expensive to retrofit.
- **Right-sizing note:** The `SyntaxTree`/`TypedModel` anti-mirroring rules and required inventory are correct but belong in a component-level spec, not the architecture doc. The five constraint families could name two evaluator modes (fire-mode, edit-mode) for mental model simplicity.

### 2026-07-17 — Comprehensive revision pass: team review feedback + innovations + new sections

- Applied all findings from three reviewers (Frank design review, George technical accuracy, Elaine readability) in a single revision pass to `docs/working/combined-design-v2.md`.
- **Navigation:** Added "How to read this document" paragraph after status block mapping §1–§3 (spine), §4–§10 (stages), §11–§14 (runtime/tooling/appendix).
- **Elaine feedback:** Folded formulaic guarantee paragraphs into stage openings for §8–§10 (Proof, Compilation Snapshot, Lowering). Kept separate labeled paragraphs for §4–§7 where the connection isn't self-evident.
- **George feedback — Parser specificity:** Added parser/TypeChecker contract boundary, error recovery shape (construct-level panic mode, `MissingNode` + `SkippedTokens`), node inventory (one type per `ConstructKind`), catalog-to-grammar mapping, anti-Roslyn guidance (no red/green trees, precedence-climbing, LL(1)), `ActionKind` dual-use note.
- **George feedback — Roslyn-bias:** Added anti-pattern for per-construct check methods in TypeChecker, clarified flow narrowing scope in ProofEngine, added anti-pattern for serialized TypedModel in Lowering, committed to flat evaluation plan (slot-addressed opcodes), stated expression evaluation is not tree-walk.
- **George feedback — Descriptor shapes:** Added brief shape definitions for `FieldDescriptor`, `StateDescriptor`, `EventDescriptor`, `ArgDescriptor`, `ConstraintDescriptor`.
- **Frank feedback:** Added proof coverage boundary (sample corpus validation requirement, fifth strategy contingency). Added initial-state satisfiability section threaded through proof/fault chain. Specified Restore recomputes computed fields BEFORE constraint evaluation. Noted `ConstraintActivation` should be cataloged. Added `Version` serialization contract. Named definition versioning as a known v1 gap. Added no-incremental-compilation model (64KB ceiling justification). Added contract digest hash specification.
- **Innovations callouts:** Added `> **Precept Innovations**` callout blocks to every major section (§2 through §14), each with 2–4 section-specific bullet points. Covered: catalog-as-spec inversion, single-act propagation, grammar generation, unified pipeline, progressive intelligence, flat evaluation plans, compile-time satisfiability, ConstraintInfluenceMap, structured "why not", inspection API, zero-drift guarantee, MCP vocabulary from catalogs, AI-first design.
