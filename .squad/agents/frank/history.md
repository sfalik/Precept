...
## Learnings
- 2026-05-11 — Empty typed-constant completion diagnosis: the lexer emits a single `TypedConstant` token for empty `''`, and the cursor-between-quotes span math is correct (`SourceSpan.EndColumn` is exclusive). The two real failure modes were elsewhere: (1) invoked completion can arrive with an empty trigger character, so `triggerCharacter is null` is too strict for Ctrl+Space routing; (2) binary peer-operand inference must step left past the current typed-constant token when completion is reopened inside the literal, or expression-site typed-constant values stay empty even though `'`-trigger inference works before the literal exists.
