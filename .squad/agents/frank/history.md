...
### 2026-05-12T00:38:55.718-04:00 — MCP DTO-free working design written

- Wrote `docs/Working/mcp-dto-free-design.md` as the canonical working-design document for the approved DTO-free MCP direction.

- Locked Shane's decision near the top of the doc: **Approach 4 (Hybrid)** is approved, there are no known programmatic consumers of catalog JSON, and implementation may proceed.

- Preserved the earlier architectural boundary: raw core serialization remains rejected; the approved path is still a curated MCP projection, now rendered as markdown for catalog tools and minimal JSON only where structure is genuinely programmatic.
...
### 2026-05-12T00:46:26.773-04:00 — Newman-ready MCP implementation plan appended

- Expanded `docs/Working/mcp-dto-free-design.md` with an execution-grade plan: exact tool signature changes, formatter-module design, compile-contract reduction, cleanup list, and test rewrite map.
- Locked the implementation boundary: catalog tools go direct-to-markdown, `precept_compile` keeps only minimal diagnostic JSON plus `ProjectDefinition` text, and no runtime / LSP / VS Code work is required.
- Explicitly called for `LanguageTool.cs` and the catalog DTO files to die once the focused tools are rewired; the old aggregate DTO surface is not to survive as a hidden fallback.

### 2026-05-12T00:53:32.916-04:00 — MCP context-window analysis completed

- Measured the current MCP payloads from the live DTO surface: legacy `precept_language` is ~401 KB, while the largest focused JSON tools (`precept_types`, `precept_operations`, `precept_domains`) already sit in the ~64-74 KB band.
- Confirmed the approved hybrid split fixes the catastrophic aggregate-call problem, but not the whole size story: `precept_types` and `precept_domains` remain large enough that repeated unscoped calls can still crowd GPT-class sessions.
- Locked the pre-ship mitigation: do **not** keep `precept_language`; add `scope` to `precept_types` and `precept_domains`; keep `precept_operations` filtered-first; `precept_compile` does not need emergency pagination yet, though a future `maxDiagnostics` cap is reasonable.
...
### 2026-05-12T05:04:03Z — George closed E2 and E3
- George shipped typed interpolated constant qualifier extraction in `8785d753`, subexpression / compound-unit qualifier propagation in `d3f5aa98`, and the ReceiveShipment parenthesization follow-up in `f4db093e`.
- samples/inventory-item.precept PRE0114 is now down to 16 remaining diagnostics; the outstanding path is the deferred exchange-rate / GrossProfit work, not the completed E2/E3 compiler slices.
