...
- **Routing family vs. `ConstructFamily` (2026-05-02):** The word "family" was used for two distinct concepts in `parser-radical.md`, both producing sets of four — a reader trap. Ruling: "routing family" (parse scope, all 12 constructs, four buckets: Header/Direct/StateScoped/EventScoped) stays as-is in §0.9. `ConstructFamily` (the catalog type for shared-leader constructs requiring disambiguation, four entries: In/To/From/On) is always called by its type name. Header and Direct constructs have unique leading keywords and have no `ConstructFamily` entry — this distinction is now explicit at both §0.3 and §3.2. Bare "family" without a qualifier is forbidden in all prose going forward.
...
- **Outcomes catalog reversal (2026-05-02):** Original section 0.8 ruling that outcomes do NOT need a catalog was reversed. The decisive argument is the `no transition` composition gap: `TokenCategory.Outcome` enumerates tokens that participate in outcomes, not outcomes themselves. `no transition` is two tokens (`No` + `Transition`) composing into one outcome — deriving that requires hardcoding composition logic, which is domain knowledge that belongs in the catalog. Outcomes now follow the same two-level pattern as Actions: `OutcomeKind` + `OutcomeMeta` + `Outcomes.cs` at the metadata level, `OutcomeNode` DU at the syntax-node level. The DU stays for parsing; the catalog supplements it for enumeration and metadata. Consumer test: `Outcomes.All` returns 3 `OutcomeMeta` entries — clean, no token-composition logic.
