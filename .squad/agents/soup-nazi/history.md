...
- 2026-04-26 Cross-catalog analyzer test infrastructure assessment: The key blocker is AnalyzerTestHelper being single-source-string only — fix is a `params string[] sources` overload (~20 lines). Three-string pattern (SharedDecls + TypesSource + OperationsSource) maps cleanly to the existing preamble-constant style. Critical constraint: stubs must NEVER include `FrozenDictionary`/`ImmutableArray` — the helper only has mscorlib+System.Runtime, and `All`/`UnaryIndex`/`BinaryIndex` must be omitted from stubs entirely (analyzers check GetMeta switch arms, not derived indexes). Cross-catalog analyzers must key on class name ("Types", "Operations") not file path, or tests will silently fail on path-keying checks. Minimal stub enums (3-5 members) are correct and deliberate — not exhaustive by design. ~8-10 tests per cross-catalog analyzer, ~260 lines per file, ~1490 total LOC for a 5-analyzer suite including the helper addition. Assessment filed to `.squad/decisions/inbox/soup-nazi-analyzer-test-infra.md`.

- 2026-04-25 Tokens + Diagnostics catalog baseline tests: `Literal` TokenCategory is shared between keyword literals (True, False — non-null text) and synthetic value tokens (NumberLiteral, StringLiteral, etc. — null text). A naïve "every token with Literal category has non-null Text" test would fail. Resolution: maintain an explicit `SyntheticTokenKindSet` HashSet of the 13 null-text kinds and exclude them from the keyword-text assertion. The SyntheticTokenKinds MemberData mirrors the same 13 kinds for the complementary null-text Theory. Also: `Diagnostics.Create` uses `string.Format`; calling it with no args throws `FormatException` for any template with placeholders. Supply 4 dummy `"x"` args for all Theory invocations — extra positional args are silently ignored and no template in the catalog needs more than 4.
