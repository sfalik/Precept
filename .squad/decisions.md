...
### 2026-04-15T17:45:00Z: Shane — Issue #96 owner decisions (5 items)
**By:** Shane (owner)
**Status:** Captured — directives for implementation

1. **No backwards compatibility.** Hard cutover — `invariant` and `assert` are removed entirely. No deprecated aliases, no transition period, no warning diagnostics for old keywords. They simply stop parsing.
2. **Rename internal types and MCP DTOs.** C# types (`PreceptInvariant` → `PreceptRule`, `StateAssertion` → etc.), MCP DTO property names, and serialization strings all rename to match the new keywords. Breaking MCP contract change is acceptable.
3. **Archive `docs/RulesDesign.md`.** Move to `docs/archive/` — it describes a design that was never implemented and is now superseded.
4. **Update all active docs.** Every reference to `invariant`/`assert` in active documentation must be updated in the implementation PR.
5. **Syntax highlighting.** `rule` and `ensure` should be highlighted in the same gold color as the `because` reason text — not in the standard keyword colors. This is a deliberate visual treatment to distinguish constraint keywords from control flow keywords.

---

### 2026-04-15: Frank — Issue #96 owner decisions locked into proposal
**By:** Frank (Lead/Architect)
**Requested by:** Shane (owner)
**Status:** Locked — incorporated into issue #96 body

Shane made 5 owner decisions after the design review ceremony for issue #96 (replace `invariant`/`assert` with `rule`/`ensure`). All decisions have been incorporated into the issue body.

1. No backwards compatibility — hard cutover.
2. Rename internal types and MCP DTOs — breaking MCP contract change accepted.
3. Archive `docs/RulesDesign.md` — move to `docs/archive/`.
4. Update all active docs in the implementation PR.
5. Syntax highlighting — gold color (`keyword.other.grammar.precept`) for constraint keywords.

---

### 2026-04-15: Frank — Update Design Principle #5 with rule/ensure vocabulary
**By:** Frank (Lead/Architect)
**Owner approval:** Shane — approved 2026-04-15
**Status:** Locked — to be applied in the issue #96 implementation PR

Principle #5 in `docs/PreceptLanguageDesign.md` updated from `invariant`/`assert` to `rule`/`ensure` vocabulary. Semantic distinction (data-truth vs movement-truth) preserved exactly. Only vocabulary changes. Applied in the same PR that implements issue #96.

---

### 2026-04-15: Frank — Issue #96 (rule/ensure keywords) — NOT ready for design review (pre-review assessment)
**By:** Frank (Lead/Architect)
**Status:** Historical — superseded by subsequent proposal revision and successful design review

Initial assessment identified three blockers: (1) RulesDesign.md factual error, (2) `rule` keyword collision with issue #8, (3) philosophy-level change requiring owner approval. All subsequently resolved before the design review ceremony.

---

### 2026-04-13: Frank — `round(number)` should return `integer`
**By:** Frank (Lead/Architect)
**Status:** Locked recommendation

`round(number)` must return `integer`, not `number`. The current signature violates the type system's own semantics. Three of four rounding functions (`floor`, `ceil`, `truncate`) return `integer` from fractional types; `round(number)` is the sole exception. `round(decimal)` already returns `integer`. The `number` overload should match. Without this fix, there is no valid path from `number` to nearest-integer rounding.

---

### 2026-04-08T04:04:35Z: Shane — EAM/maintenance domain directive
**By:** Shane (via Copilot)
**Status:** Captured

Include the EAM / maintenance domain in the sample-research program, with maintenance-related samples such as work orders as a priority lane. Shane's domain background at Ivara / Bentley Asset Reliability / APM should inform the realism standard for this lane.

---

### 2026-04-08T04:06:40Z: Shane — EAM research as additional lane
**By:** Shane (via Copilot)
**Status:** Captured

Treat EAM / maintenance research and sample coverage as an additional lane without detracting from or reprioritizing the work already in flight.

---

### 2026-04-08T05:11:44Z: Shane — Sample program on hold
**By:** Shane (via Copilot)
**Status:** Captured

Put the sample-program work on hold until the new language features are implemented, then resume from the current research and planning base.

---

### 2026-04-08: Frank — State-graph shape taxonomy and insurance depth for sample portfolio
**By:** Frank (Lead/Architect)
**Status:** Research delivered — awaiting Shane review

Nine-shape state-graph taxonomy established. Current 21-sample corpus covers only 5 of 9. Four enterprise-governance shapes (AG, AL, EL, CP) completely absent. Portfolio priority: shape coverage outranks domain count. Insurance must deepen to three samples. State-scoped field editability under-demonstrated. Shape tagging convention (`# Shape:` header comment) proposed.

---

### 2026-04-08: Peterman — Paired domain bundles for sample realism
**By:** J. Peterman
**Status:** Research conclusion — awaiting integration

Next realism-focused sample wave should be **paired domain bundles**: one governed case/lifecycle sample plus one companion governed entity or reference-data sample from the same ecosystem. Prioritize bundles like prior-authorization appeal + payer/provider organization, KEV remediation case + KEV catalog entry, purchase-order response + vendor master.

---

### 2026-04-08: Peterman — EAM/Maintenance as Tier 1 sample domain
**By:** J. Peterman
**Status:** Research conclusion — for team awareness and sample planning

EAM/Maintenance/APM confirmed as Tier 1 domain (24/25 domain-fit score). Recommended 6-candidate sample suite with 2 at P0: Corrective Maintenance Work Order (rewrite) and Permit-to-Work. Fills Authorization-Gate shape gap. Shane has direct domain background from Ivara/Bentley.

---

### 2026-04-08: Steinbrenner — EAM multi-lane portfolio planning
**By:** Steinbrenner (PM)
**Status:** Proposed — awaiting integration

Treat EAM as multi-lane portfolio domain: `maintenance-work-order` rewrite as first flagship (W3 authorization-gated execution), `asset-record-lifecycle` as first hybrid companion (H1), `failure-code-catalog` and `preventive-maintenance-template` reserved for #22 lane. Move `maintenance-work-order` into first rewrite wave; defer `utility-outage-report`.

---

### 2026-04-08: Steinbrenner — Two-axis sample-planning taxonomy
**By:** Steinbrenner (PM)
**Status:** Proposed — awaiting integration

Adopt two-axis taxonomy: (1) contract archetype (workflow, hybrid, stateless/entity) and (2) lifecycle shape. Rewrite-first order locked. Next net-new workflow slots target four missing shape families. Add 2-3 hybrid samples before balanced corpus. Reserve 4-6 stateless/entity slots for #22-era samples. 30 near-term / mid-30s operating band / 42 hard upper bound ceiling model.
