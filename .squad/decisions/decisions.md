...
**`DeclaredQualifierMeta.Dimension`** (in `DeclaredQualifierMeta.cs`) carries `DimensionName` (e.g., `"mass"`). The preposition is `TokenKind.Of`. This is produced when a field is declared with `of '<dimension>'`.
...
**`QualifierMismatch`** (diagnostic code 68): "Value does not match the '{0}' qualifier on field '{1}'". This is the appropriate code for "bound unit doesn't match the field's pinned unit" (Path A — `Unit` qualifier). Already declared in `DiagnosticCode.cs` and cataloged in `Diagnostics.cs`.
...
Also add the `ValidateMinMaxBoundQualifier` private static method to `TypeChecker.cs`. Verify the `UcumParsedUnit` fully-qualified name by checking the `using` directives in `TypeChecker.cs` or adding the appropriate using.
...
### D. `docs/language/precept-language-spec.md` — modifier applicability table (~line 1495)
...
## What Kramer Does NOT Need to Touch (revised)
...
After the existing `default` modifier resolution block (find: `ctx.Fields[i] = ctx.Fields[i] with { DefaultExpression = resolved };`), add:
...
Also add the `ValidateMinMaxBoundQualifier` private static method alongside the other `Map*Qualifier` and `DeriveUnitDimensionName` helpers:
...
**Note on `UcumParsedUnit` namespace:** Kramer must verify the exact namespace for `UcumParsedUnit` (it is `Precept.Language.Ucum.UcumParsedUnit` per `UcumParsedUnit.cs`) and confirm the using directives in `TypeChecker.cs` cover it, or add `using Precept.Language.Ucum;`.
