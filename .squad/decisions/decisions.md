# George — Slice 3 Semantic Model DU Complete

Date: 2026-07-17

## What
George completed the semantic-model discriminated-union work for Slice 3 at commit `7c49f9c7`.

## Why
The DU shape is the semantic baseline that later construction-row slices and reviews now build on.

## Tradeoff
This captures the completion decision only; file-level detail stays in the original work record. Source: deleted inbox file `george-slice3-complete.md`.

# George — Slice 6 Graph Analyzer Construction Awareness Complete

Date: 2026-07-17

## What
George completed Slice 6 so the graph analyzer is aware of construction rows.

## Why
Construction handling could not remain parser/type-checker-only; graph analysis had to understand the same semantics.

## Tradeoff
The entry captures the outcome and key decisions, while file-level implementation detail stays in the original slice note. Source: deleted inbox file `george-slice6-complete.md`.
...
# User Directive — Catalog-Driven Completions Routing

Date: 2026-05-16T09:02:56

## What
Route Slices 2 and 3 of the catalog-driven completions refactor to Kramer, with Frank reviewing after each slice before the next slice starts.

## Why
This preserves explicit ownership and review order for the refactor follow-on work.

## Tradeoff
The routing decision is people/process state, not product behavior. Source: deleted inbox file `copilot-directive-completions-routing.md`.
...
# Frank — Graph Analyzer Coverage Should Count Construction Event Rows

Date: 2026-05-16

## What
Confirm that graph edge generation is already complete: only `TransitionRows` should produce graph edges. The real gap is coverage accounting — PRE0081 and `GraphEvent.IsInitial` must also treat construction rows in `EventHandlers` as handled.

## Why
`EventRows` and construction rows never model state-to-state movement, so they should not generate edges. But construction-only events are still semantically handled and should not look unhandled just because they bypass `TransitionRows`.

## Tradeoff
Slice 8b should apply the PRE0081 suppression and `GraphEvent.IsInitial` fix surgically rather than widening the graph model. Source: deleted inbox file `frank-graph-analyzer-all-event-rows.md`.

# User Directive — Spike Mode Operating Rules

Date: 2026-05-16

## What
Work remains on branch `spike/Precept-V2-Radical` in spike mode: no PR creation, no gate enforcement, and direct commits are allowed.

## Why
Shane explicitly suspended the normal draft-PR implementation gate for this spike branch.

## Tradeoff
These rules are spike-local and should not be treated as the default repo workflow. Source: deleted inbox file `copilot-directive-spike-mode.md`.

# Frank — Catalog-Driven Completions Architecture

Date: 2026-05-16

## What
Lock the completions refactor to the incremental catalog-driven path: keep catalog-based item generation, add explicit slot vocabulary metadata, and replace `SlotContext` routing with a structural slot-position resolver.

## Why
The recurring completion bugs come from heuristic cursor routing, not from missing catalog vocabularies. Slot metadata already describes construct shape closely enough to become the primary routing axis.

## Tradeoff
Hard cases still need targeted procedural logic, but the routing model shifts to catalog metadata instead of continuing one-off comma/arrow/post-slot patches. Source: deleted inbox file `frank-completions-architecture.md`.

# Frank — Catalog-Driven Completions Implementation Plan

Date: 2026-05-16

## What
Record the three-slice execution plan: (1) add `SlotVocabulary`, `IsList`, `IsChainable`, and `ItemIntroducerToken` to `ConstructSlot`; (2) introduce `SlotPositionResolver` plus `ResolvedSlotPosition`/`SlotPhase` as a shadow path; (3) cut `CompletionHandler` over, delete `SlotContext`, and consolidate generators.

## Why
The staged plan converts completions from heuristic token inspection to catalog metadata while keeping each slice independently testable and reversible.

## Tradeoff
`decisions.md` stores only the decision summary. The original long-form implementation plan came from deleted inbox file `frank-completions-plan.md`.

# Frank — Construction Rows Should Not Require `initial` on the Row

Date: 2026-05-16

## What
Treat row-level `initial` as removable syntactic noise and unify construction/event rows at the parse surface, with construction classification derived from the resolved event's `IsInitial` flag in the type checker.

## Why
`initial` is only serving parser disambiguation today. The type checker already normalizes both row families and already resolves the event symbol, so the semantic source of truth belongs there.

## Tradeoff
This is a breaking syntax cleanup on the spike branch and moves one guard diagnostic from parser-time to type-check-time. Source: deleted inbox file `frank-initial-row-analysis.md`.

# Frank — Slice 1 Grammar Foundations Complete

Date: 2026-05-16

## What
Grammar foundations landed for the construction-row work: new construction/reject construct kinds and success/reject outcome forms were added, `EventHandler` was renamed to `EventRow`, and construct metadata/disambiguation tables were updated.

## Why
These structural grammar changes establish the parser/catalog vocabulary needed for later construction-row slices.

## Tradeoff
This entry records the shipped foundation only; later slices still own the semantic/runtime follow-through. Source: deleted inbox file `frank-slice1-complete.md`.

# Frank — Slice 2 Parser Routing Complete

Date: 2026-05-16

## What
Parser routing for the construction-row spike was completed and recorded as the next shipped slice after the grammar-foundation work.

## Why
The routing slice closes the parser-side classification path needed before later semantic/runtime slices can rely on the new construct shapes.

## Tradeoff
Downstream validation and runtime behavior still depended on later reviewed slices. Source: deleted inbox file `frank-slice2-complete.md`.

# Frank — Slice 3 Review Gate Approved

Date: 2026-05-16

## What
Frank approved Slice 3 at commit `7c49f9c7`, clearing the semantic-model DU work to stand as the accepted implementation baseline.

## Why
The review gate records that the slice was accepted and can be treated as the authoritative base for follow-on work.

## Tradeoff
Approval included only minor non-blocking notes; any broader cleanup stays outside this gate. Source: deleted inbox file `frank-slice3-approved.md`.

# Frank — Slice 4 Structural Review Approved

Date: 2026-05-16

## What
Frank approved Slice 4 structural/type-checker work at commit `f8b1febd`.

## Why
This locks the construction-row structural validation behavior and confirms the tests/design notes were acceptable for the spike.

## Tradeoff
The review records acceptance of the structural slice, not completion of the full construction-row program. Source: deleted inbox file `frank-slice4-approved.md`.

# Frank — Test.precept Line 8 Guidance

Date: 2026-05-16

## What
Keep `samples/Test.precept` on the pre-8b syntax (`on create initial`) until Slice 8b lands, and update PRE0146/PRE0147 wording when the new syntax ships.

## Why
The current four-diagnostic cascade is expected while the parser still relies on row-level `initial`, and a broken sample should not become the repo's canonical usage reference.

## Tradeoff
Slice 8b must carry sample, diagnostic-message, and regression-test follow-through together. Source: deleted inbox file `frank-test-precept-line8-analysis.md`.

# George — Slice 4 Structural Validation Complete

Date: 2026-05-16

## What
Slice 4 structural validation landed in `TypeChecker.Validation.Structural.cs` with PRE0145, PRE0146, PRE0147, construction-row-aware PRE0092 behavior, and dedicated structural tests.

## Why
Stateful-precept construction semantics needed explicit diagnostics and validation coverage before later graph/runtime slices could build safely.

## Tradeoff
The compatibility note remains relevant: this was a structural slice, not the end of the construction-row implementation. Source: deleted inbox file `george-slice4-complete.md`.

# George — Slice 5 Field State Complete

Date: 2026-05-16

## What
George completed Slice 5 for construction-aware field-state handling and recorded the shipped result plus validation outcome.

## Why
Construction semantics needed field-state behavior aligned with the earlier parser and structural slices.

## Tradeoff
This is a milestone record; detailed mechanics remain in the original slice note. Source: deleted inbox file `george-slice5-complete.md`.

# George — Slice 7 Proof Engine Construction-Row Context

Date: 2026-05-16

## What
Lock three Slice 7 decisions at commit `f1eb3cab`: reuse `EventHandlerContext` rather than inventing a new context type, use `ProveAllowingDiagnostics` for the targeted test path, and defer event-arg-ref narrowing in construction guards.

## Why
These constraints keep the proof-engine slice narrow and aligned with the spike's existing semantic shapes.

## Tradeoff
Construction-guard event-arg narrowing remains follow-up work outside this slice. Source: deleted inbox file `george-slice7-decisions.md`.

# George — Slice 8 Runtime Created Outcome and Fire-Once Rules

Date: 2026-05-16

## What
Lock the Slice 8 runtime decisions at commit `d95fff84`: store `SemanticIndex` on `Precept`, keep spike-level `Create()` focused on `EventHandlers`, skip guarded construction rows for now, expose `FiredArgs.Empty`, delegate `Version.AvailableEvents`, and enforce fire-once via the initial-event check.

## Why
These decisions define the smallest viable runtime surface for construction on the spike branch without prematurely widening the API.

## Tradeoff
Guarded construction execution stays deferred beyond the spike-level `Create()` path. Source: deleted inbox file `george-slice8-decisions.md`.

# Kramer — From-Clause Comma Continuation Routing

Date: 2026-05-16

## What
Recover the owning `StateTarget` slot after `from off, ` so completions stay on the same list, suppress `any`, and filter already-listed states.

## Why
The bug was a routing failure on comma punctuation, not a missing completion vocabulary.

## Tradeoff
This durable rule is specific to state-target list continuation, though it motivated the broader slot-level comma work. Source: deleted inbox file `kramer-from-clause-completions.md`.

# Kramer — Slot-Level Comma Continuation Routing

Date: 2026-05-16

## What
Treat comma as continuation punctuation for the owning semantic list across state targets, state declaration entry lists, and `modify`/`omit` field lists.

## Why
The remaining comma bugs all came from routing punctuation outside the active list slot instead of staying inside the slot's vocabulary.

## Tradeoff
The fix broadens the routing contract and requires list-aware filtering for each affected slot family. Source: deleted inbox file `kramer-slot-level-comma-fix.md`.

# Kramer — Trailing Action-Chain Arrow Routing

Date: 2026-05-16

## What
Run structural action-chain continuation recovery before construct-aware slot checks so a fresh trailing `->` still resolves to action-verb completions.

## Why
Recovered rows can extend through the new arrow while the `ActionChainSlot` span still ends at the prior action, which makes construct-aware checks alone under-classify valid continuation sites.

## Tradeoff
The recovery stays intentionally narrow: it requires a prior action verb and a prior same-or-deeper-indented arrow. Source: deleted inbox file `kramer-trailing-arrow-fix.md`.

# Frank — Slice 2 Review Gate Approved

Date: 2026-05-16

## What
Frank approved Slice 2 at commit `6842353e`, confirming `SlotPositionResolver` is architecturally sound as the catalog-driven shadow-path backbone and that the test matrix is sufficient for the Slice 3 cutover gate.

## Why
The review clears the structural resolver work to stand as the accepted baseline before `CompletionHandler` routing moves off `SlotContext`.

## Tradeoff
Slice 3 still needs to decide which legacy micro-contexts are preserved or intentionally collapsed, and the action-chain expression-introducer check should become catalog-derived once the slot metadata can express that distinction. Source: deleted inbox file `frank-slice2-review.md`.

# Kramer — Slice 2 Slot Position Resolver Complete

Date: 2026-05-16

## What
Kramer completed Slice 2 of the catalog-driven completions refactor at commit `6842353e`, adding `tools/Precept.LanguageServer/SlotPositionResolver.cs` and `test/Precept.LanguageServer.Tests/SlotPositionResolverTests.cs` as the additive shadow-path resolver and validation matrix.

## Why
Slice 2 establishes the structural slot-position routing backbone for the catalog-driven completions cutover while keeping live completion behavior on the legacy path until Slice 3 is ready.

## Tradeoff
Legacy-only micro-contexts such as `AfterValueName`, `InArgDefault`, `InSetAssignment`, and some pre-disambiguation/post-keyword handoffs still remain with `SlotContextResolver` until Slice 3 decides whether to preserve or intentionally collapse them. Source: deleted inbox file `kramer-slice2-findings.md`.
...
# User Directive — Model Selection Policy

Date: 2026-05-15T23:25:27

## What
Do not use `claude-opus-4.5`. Standard work should default to `claude-sonnet-4.6`, and high-complexity work should use `claude-opus-4.6`.

## Why
This captures Shane's explicit model policy for squad work and aligns the repo memory with the `.squad/config.json` update.

## Tradeoff
The directive is operational, not architectural. Source: deleted inbox file `copilot-directive-2026-05-15T23-25-27.md`.
...
# Decision: slice-docs-samples Complete

**Date:** 2026-05-17T18:06:33Z
**Author:** Frank (Lead/Architect)
**Status:** Closed

## Summary

Constructor semantics documentation closeout (`slice-docs-samples`) is complete. All documentation and sample surfaces now reflect the final shipped syntax and semantics.

## Changes Made (commit `8a2bae37`)

1. **`samples/Test.precept`** — Fixed syntax: split `event create initial, start, stop, reset` into two proper declarations (`event create initial` + `event start, stop, reset`). The `initial` modifier terminates an event declaration line; comma-separated events after it were a parse error.

2. **`docs/language/precept-language-spec.md`** — Two surgical additions:
   - §3.8 (Stateless/stateful cross-validation): Explicit note that construction rows are exempt from PRE0092. The type checker identifies them via `resolvedEvent.IsInitial` before the stateless-handler rule fires.
   - §5 (Proof Engine): Explicit PRE0115 construction exemption for initial-state satisfiability. When construction rows exist, the entity never inhabits the initial state with only defaults, so the satisfiability check is inapplicable.

3. **`docs/runtime/runtime-api.md`** — Already complete from prior slice closeout. No changes needed.

4. **`CHANGELOG.md`** — Already has the full constructor semantics entry (Slices 1–12) under `[Unreleased]`. No changes needed.

## Verification

- Pattern A samples (`loan-application`, `parcel-locker-pickup`, `clinic-appointment-scheduling`) validated syntactically correct. MCP `precept_compile` still shows stale PRE0092 (known deployment drift — actual compiler has the fix per commit `57433ce1`, verified by 5798/5798 green test suite).
- Test.precept syntax fixed and verified through MCP compile (remaining errors are pre-existing PRE0076 on `USD/each` and stale PRE0092 deployment drift).

## Conclusion

Constructor semantics implementation is **fully closed**. All 12 slices shipped, all downstream surfaces (grammar, completions, hover, semantic tokens, MCP DTO, diagnostics, proof engine, docs, samples, CHANGELOG) are synchronized.
